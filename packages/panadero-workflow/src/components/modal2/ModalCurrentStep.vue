...
// Computed properties - USE SSOT DIRECTLY
// ✅ FIX - Convert 1-based to 0-based for array access
...
// Determine display mode - USE SSOT DIRECTLY
...
// Line 64-66: Update the logic for 1-based indexing
  ...
  <div class="flex-1 overflow-y-auto p-2 mt-0">
    ...
    <div class="mb-4">
      <div class="flex items-center justify-between">
        <h3 :style="{ fontSize: scaling.font.body }" class="font-semibold text-gray-900 dark:text-white">
          Input Section
        </h3>
        <button 
        ...
        <div v-if="activeWorkflow">
          ...
          <div class="ml-4 text-sm space-y-1">
            ...
            <div class="font-bold text-blue-600 dark:text-blue-400">
              Current Step: {{ activeWorkflow.value.currentStep }}/{{ activeWorkflow.value.steps?.length || 0 }}
            </div>
            <div>Viewed Step: {{ props.viewedStep }}</div>
            <div>Module: {{ activeWorkflow.value.module }}</div>
            <div>Template ID: {{ activeWorkflow.value.template_id }}</div>
            <div>Created By: {{ activeWorkflow.value.created_by }}</div>
            <div>Workflow Nr: {{ activeWorkflow.value.workflowNr }}</div>
            <div>Started At: {{ activeWorkflow.value.started_at ? new Date(activeWorkflow.value.started_at).toLocaleString() : 'Not started' }}</div>
            <div>Completed At: {{ activeWorkflow.value.completed_at ? new Date(activeWorkflow.value.completed_at).toLocaleString() : 'Not completed' }}</div>
          </div>
        </div>
        ...
        <div class="text-gray-500 dark:text-gray-400">
          ...
          <div class="mt-4">
            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
              ...
              Current Step: {{ activeWorkflow.value.currentStep }}
            </span>
          </div>
        </div>
      </div>
