<script setup>
import { ref, computed } from 'vue'
import { useComplianceStore } from '../../stores/complianceStore.js'
import { useScaling } from '../../../../shared/composables/useScaling.js'
import { useComplianceInfoBoxes } from '../../composables/useComplianceInfoBoxes.js'

const store = useComplianceStore()
const { fontSizes, scalingStyles } = useScaling()
const { infoBoxes } = useComplianceInfoBoxes()

const selectedInfoBox = ref('policies')

const currentInfoBox = computed(() => {
  return infoBoxes.value.find(box => box.id === selectedInfoBox.value) || infoBoxes.value[0]
})
</script>

<template>
  <div class="info-section">
    <!-- Header -->
    <div class="mb-8">
      <h2 :style="{ fontSize: `${fontSizes.large}px` }" class="text-2xl font-bold text-gray-900 dark:text-white mb-2">
        ERP Compliance Module
      </h2>
      <p :style="{ fontSize: `${fontSizes.base}px` }" class="text-gray-600 dark:text-gray-400">
        Comprehensive compliance management with regulatory oversight, audit trails, risk management, and AI-powered analysis
      </p>
    </div>

    <!-- Info Box Selector -->
    <div class="mb-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <button
          v-for="box in infoBoxes"
          :key="box.id"
          @click="selectedInfoBox = box.id"
          :class="[
            'p-4 rounded-lg border-2 transition-all duration-200 text-left',
            selectedInfoBox === box.id
              ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20'
              : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600'
          ]"
        >
          <div class="flex items-center mb-2">
            <i :class="[box.icon, `text-${box.color}-500`]" class="text-xl mr-3"></i>
            <h3 class="font-semibold text-gray-900 dark:text-white">{{ box.title }}</h3>
          </div>
          <p class="text-sm text-gray-600 dark:text-gray-400">{{ box.description }}</p>
        </button>
      </div>
    </div>

    <!-- Selected Info Box Content -->
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
      <div class="flex items-center mb-4">
        <i :class="[currentInfoBox.icon, `text-${currentInfoBox.color}-500`]" class="text-2xl mr-3"></i>
        <h3 :style="{ fontSize: `${fontSizes.medium}px` }" class="text-xl font-semibold text-gray-900 dark:text-white">
          {{ currentInfoBox.title }}
        </h3>
      </div>
      
      <p :style="{ fontSize: `${fontSizes.base}px` }" class="text-gray-600 dark:text-gray-400 mb-6">
        {{ currentInfoBox.description }}
      </p>

      <!-- Stats -->
      <div v-if="currentInfoBox.stats" class="mb-6">
        <h4 :style="{ fontSize: `${fontSizes.small}px` }" class="font-medium text-gray-900 dark:text-white mb-3">
          Current Statistics
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div
            v-for="stat in currentInfoBox.stats"
            :key="stat.label"
            class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3"
          >
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ stat.label }}</p>
            <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ stat.value }}</p>
          </div>
        </div>
      </div>

      <!-- Features -->
      <div v-if="currentInfoBox.features" class="mb-6">
        <h4 :style="{ fontSize: `${fontSizes.small}px` }" class="font-medium text-gray-900 dark:text-white mb-3">
          Key Features
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div
            v-for="feature in currentInfoBox.features"
            :key="feature"
            class="flex items-center"
          >
            <i class="fas fa-check text-teal-500 mr-2"></i>
            <span :style="{ fontSize: `${fontSizes.small}px` }" class="text-gray-600 dark:text-gray-400">{{ feature }}</span>
          </div>
        </div>
      </div>

      <!-- Status -->
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <div class="w-3 h-3 bg-teal-500 rounded-full mr-2"></div>
          <span :style="{ fontSize: `${fontSizes.small}px` }" class="text-gray-600 dark:text-gray-400">
            Status: {{ currentInfoBox.status || 'Active' }}
          </span>
        </div>
        <div class="text-right">
          <span :style="{ fontSize: `${fontSizes.small}px` }" class="text-gray-500 dark:text-gray-400">
            Last updated: {{ new Date().toLocaleDateString() }}
          </span>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.info-section {
  padding: 0;
}
</style>
