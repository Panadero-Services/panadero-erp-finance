  ...
  try {
    const result = await store.executeAgentPrompt(promptId, {
      hrData: {
        ...
        activeVacancies: store.getActiveVacancies.length,
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an HR management AI assistant. Provide helpful insights about human resources, recruitment, employee management, and organizational development.'
        },
        {
          role: 'user',
...
const callClaude = async (prompt, context, config) => {
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `You are an HR management AI assistant. ${prompt}\n\nContext: ${JSON.stringify(context)}`
      }]
    })
  })
...
const callOllama = async (prompt, context, config) => {
  ...
  const response = await fetch(`${baseUrl}/api/generate`, {
    ...
    body: JSON.stringify({
      model: config.model,
      prompt: `You are an HR management AI assistant. ${prompt}\n\nContext: ${JSON.stringify(context)}`,
      stream: false,
      options: {
        temperature: config.temperature
      }
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p :style="{ fontSize: `${fontSizes.base}px` }" class="text-gray-600 dark:text-gray-400">
            Leverage AI to analyze and optimize your HR processes and recruitment
          </p>
        </div>
