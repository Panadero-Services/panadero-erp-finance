<!--
  ...
  @version 1.0.13
  @date 17-Sep-2025
  @description Manage outgoing stock orders, shipments, and deliveries
-->
<script setup>
import { ref, computed, onMounted } from 'vue'
import { useOutgoing } from '../composables/useOutgoing.js'
  ...
  <div class="outgoing" :class="darkModeClasses.container">
    ...
    <div class="mb-6">
      ...
      <p :style="scalingStyles.subtitleFontSize" :class="darkModeClasses.textSecondary" class="mt-2">
        Manage outgoing stock orders, shipments, and deliveries
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      ...
      <InventoryValueCard
        title="Shipping"
        :value="getOrdersByStatus('shipping').length"
      ...
      <InventoryValueCard
        title="Total Value"
        :value="formatCurrency(getTotalOutgoingValue)"
          ...
          <tbody :class="[darkModeClasses.table, 'divide-y', darkModeClasses.border]">
            <tr v-for="order in filteredOrders" :key="order.id" :class="[darkModeClasses.tableRow, 'hover:bg-gray-50']">
              ...
              <td :style="scalingStyles.textFontSize" class="px-4 py-4 whitespace-nowrap">
                <StatusBadge 
                  :status="order.status === 'planned' ? 'info' : order.status === 'shipping' ? 'warning' : 'success'"
      ...
      <div :class="[darkModeClasses.modal, 'rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto']">
        ...
        <div v-if="selectedOrder" class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label :style="scalingStyles.label" :class="darkModeClasses.text" class="block text-sm font-medium">Customer</label>
              <p :style="scalingStyles.textFontSize" :class="darkModeClasses.text" class="text-sm">{{ selectedOrder.customer }}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <StatusBadge 
                :status="selectedOrder.status === 'planned' ? 'info' : selectedOrder.status === 'shipping' ? 'warning' : 'success'"
            ...
            <table class="w-full">
              <thead :class="darkModeClasses.tableHeader">
                <tr>
                  ...
                  <th :style="scalingStyles.tableHeader" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Shipped</th>
                  <th :style="scalingStyles.tableHeader" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody :class="[darkModeClasses.table, 'divide-y', darkModeClasses.border]">
                <tr v-for="item in selectedOrder.items" :key="item.sku" :class="[darkModeClasses.tableRow, 'hover:bg-gray-50']">
                  ...
                  <td :style="scalingStyles.textFontSize" :class="darkModeClasses.text" class="px-4 py-4 whitespace-nowrap text-sm">
                    {{ item.shipped }} {{ item.unit }}
                  </td>
                  <td :style="scalingStyles.textFontSize" :class="darkModeClasses.text" class="px-4 py-4 whitespace-nowrap text-sm">
                    <InventoryButton
                      @click="shipItem(selectedOrder.id, item.sku, item.quantity - item.shipped)"
                      size="sm"
                      variant="success"
                      :style="scalingStyles.button"
                      :disabled="item.shipped >= item.quantity"
                    >
