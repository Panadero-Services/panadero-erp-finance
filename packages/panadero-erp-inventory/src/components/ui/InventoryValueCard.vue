...
<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
    <div class="flex items-center justify-between">
      <div class="flex-1">
        <p :style="scalingStyles.smallFontSize" class="font-medium text-gray-600 dark:text-gray-400 mb-1">
          {{ title }}
        </p>
        <p :style="scalingStyles.titleFontSize" class="font-bold text-gray-900 dark:text-white">
          {{ value }}
        </p>
        <p v-if="subtitle" :style="scalingStyles.smallFontSize" class="text-gray-500 dark:text-gray-400 mt-1">
          {{ subtitle }}
        </p>
      </div>
      ...
      <div v-if="icon" class="flex-shrink-0">
        <div class="w-12 h-12 rounded-lg flex items-center justify-center" :class="`bg-${color}-50 dark:bg-${color}-900/20`">
          <i :class="[icon, colorClasses]" :style="scalingStyles.iconSize" class=""></i>
        </div>
      </div>
    </div>
    
    <div v-if="trend !== 0" class="mt-4 flex items-center">
      <i :class="trendIcon" :style="scalingStyles.iconSizeSmall" class="mr-1"></i>
      <span :style="scalingStyles.smallFontSize" class="font-medium" :class="trend > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
        {{ trendText }}
      </span>
      <span :style="scalingStyles.smallFontSize" class="text-gray-500 dark:text-gray-400 ml-1">vs last period</span>
    </div>
  </div>
</template>
