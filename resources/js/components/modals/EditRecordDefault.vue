...
// State & Refs
const table = ref(props.table || 'records');
...
const boolFields = ref([
  'is_active', 'is_archived', 'is_featured', 'is_published', 'is_public', 'is_locked'
]);
      ...
      <div class="h-full flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
          <form @submit.prevent="submit" class="grid grid-cols-8 gap-4">
            ...
            >
              <!-- Label -->
              <label :for="f" :class="_input.label">
                {{ f.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }}
                <span v-if="getRuleText(f)" :class="_input.info">
                  ({{ getRuleText(f) }})
                </span>
              </label>

              <!-- Boolean Switch -->
              <div v-if="boolFields.includes(f) && superSelfAdmin" class="mt-1">
              ...
              <div v-else-if="!readOnlyFields.includes(f) && superSelfAdmin">
                ...
                <input
                  v-else
                  :type="typeof form[f] === 'number' ? 'number' : 'text'"
                  v-model="form[f]"
                  :class="[
                    'mt-1 block w-full rounded-md shadow-sm sm:text-xs',
                    isInvalid(f)
