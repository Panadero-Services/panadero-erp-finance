...
import EditContractParameterModal from "@/web3/EditContractParameterModal.vue";
...
const _walletDev = ref(false);
const _web3DbDev = ref(false);
const _tokenDev = ref(false);
const _contractDev = ref(false);
</script>

<template>

  <!--... DEVELOPER SECTION  storeVars    <div @click="_walletDev = !_walletDev" :class="_walletDev ? 'text-green-600' : 'text-gray-600'">wallet:<div v-if="_walletDev" class="text-black dark:text-white">{{wallet}}</div></div>
    <div @click="_web3DbDev = !_web3DbDev" :class="_web3DbDev ? 'text-green-600' : 'text-gray-600'">web3Db:<div v-if="_web3DbDev" class="text-black dark:text-white">{{web3Db}}</div></div>
    <div @click="_tokenDev = !_tokenDev" :class="_tokenDev ? 'text-green-600' : 'text-gray-600'">token:<div v-if="_tokenDev" class="text-black dark:text-white">{{token}}</div></div>
    <div @click="_contractDev = !_contractDev" :class="_contractDev ? 'text-green-600' : 'text-gray-600'">contract:<div v-if="_contractDev" class="text-black dark:text-white">{{contract}}</div></div>
  -->
...
<div class="max-w-screen mx-4">
  ...
  <div class="grid grid-cols-1 l:grid-cols-2 2xl:grid-cols-4 text-right place-content-end my-2 ">
    <div class="text-3xl font-semibold text-left ml-2">Contracts</div>
    ...
    <div class="">
      <button type="button" @click="loadContract(selected.id, true)"  class="rounded-md rounded-md bg-gray-50 dark:bg-gray-900 dark:text-gray-300 border-2 dark:border-gray-600 dark:hover:bg-indigo-400 dark:hover:text-black px-2 py-2 m-1 text-xs text-gray-700 shadow-md hover:bg-indigo-100 hover:text-indigo-700">Get Contract</button>
      <button type="button" @click="reset" class="rounded-md rounded-md bg-gray-50 dark:bg-gray-900 dark:text-gray-300 border-2 dark:border-gray-600 dark:hover:bg-indigo-400 dark:hover:text-black px-2 py-2 m-1 text-xs text-gray-700 shadow-md hover:bg-indigo-100 hover:text-indigo-700">Reset</button>
    </div>
  ...
  <div class="grid grid-cols-1 l:grid-cols-2 2xl:grid-cols-4 text-right place-content-end my-1">

  </div>

  <div class="grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 text-gray-700 font-semibold min-h-[570px]">

    <div class="lg:col-span-3 xl:col-span-4 ml-2 border-t-2 dark:border-gray-600">
      <dl class="mt-2 grid grid-cols-3 gap-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-9">
...
-->
    ...
    <div class="px-1 lg:px-0 lg:pt-1 border-t-2 dark:border-gray-600 col-span-1 mx-4">

  <!--... Contracts SELECTOR 
-->

      <div class="text-xs text-gray-700 font-semibold my-1">
        <span class=" leading-2 ml-0 mb-3">CONTRACT SELECTOR {{contractCounter}}:{{counter}}</span>
      </div>

      <div class="">
        <Listbox as="div" v-model="selected" class="relative mt-0 ml-1" @click="hotLoadingFields(selected.id)">
          <ListboxButton class="relative cursor-default rounded-md bg-white dark:bg-gray-800 dark:text-indigo-300 px-3 py-2 xl:px-3.5 xl:py-2.5 text-xs xl:text-sm font-semibold text-indigo-700  w-full shadow-sm ring-1 ring-inset ring-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-700 sm:text-sm sm:leading-6">
          ...
          <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
            <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-black py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              <ListboxOption as="template" v-for="webrecord in web3Db.records" :key="webrecord.id" :value="webrecord" v-slot="{ active, selected }"  >
                <li :class="[active ? 'bg-indigo-700 text-white' : 'text-gray-900 dark:bg-gray-900 dark:text-gray-300', 'relative cursor-default select-none py-2 pl-3 pr-9']">
      ...
      <div class="max-w-lg">
        ...
        <div class="text-xs text-gray-700 font-semibold">
          <span v-for="(r, index) in abiHumanView">
              <button @click="resolveLine(web3Record,index+1,resolveFunctionName(r),r,1,1)" class="flex-none rounded-md bg-gray-50 dark:bg-gray-900 dark:text-gray-400 border-2 dark:border-gray-600 dark:hover:bg-indigo-400 dark:hover:text-black px-2 py-1.5 m-1 text-xs text-gray-700 shadow-md hover:bg-indigo-100 hover:text-indigo-700">{{index}}{{resolveFunctionName(r)}}</button>
          </span>
        </div>
      </div>
...
-->
        ...
        <div class="text-xs text-gray-700 dark:text-gray-400 font-semibold my-3 dark:border-gray-600 border-t-2 mt-6">
        ...
        <form class="my-1 max-w-md">
          <div>
            <input v-if="parameters>0" v-model="parameter[0]" class="ml-1 mt-1 flex-auto rounded-md border-0 px-1 py-2 text-gray-900 placeholder:text-gray-400 dark:bg-gray-700 dark:text-gray-300 sm:text-xs sm:leading-1 w-60 xl:w-80" placeholder="Parameter1" />
          </div>
          <div>
            <input v-if="parameters>1" v-model="parameter[1]" class="ml-1 mt-2 flex-auto rounded-md border-0 px-1 py-2 text-gray-900 placeholder:text-gray-400 dark:bg-gray-700 dark:text-gray-300 sm:text-xs sm:leading-1 w-60 xl:w-80" placeholder="Parameter2" />
          </div>
          <div>
            <input v-if="parameters>2" v-model="parameter[2]" class="ml-1 mt-2 flex-auto rounded-md border-0 px-1 py-2 text-gray-900 placeholder:text-gray-400 dark:bg-gray-700 dark:text-gray-300 sm:text-xs sm:leading-1 w-60  xl:w-80" placeholder="Parameter3" />
          </div>
        </form>
...
-->
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-5 text-gray-700 dark:text-gray-500 font-semibold  min-h-[170px] ">

  <div class="col-span-4 ml-4 mt-6 min-h-[120px] dark:border-gray-600 border-t-2" >
      <div class="mt-4">
      ...
      <div class="text-left mt-6 ml-4 flex text-xs text-gray-700 dark:text-gray-400">

        <span class="text-xs font-semibold mb-3">SELF AUTONAME RESOLVE</span>
        <input type="checkbox" v-model="autoNameResolve" class="mx-2">

        <span class=" font-semibold mb-2 ml-3">EVENTHANDLERS</span>
        <input type="checkbox" v-model="eventHandlers" class="mx-2">

        <span class="font-semibold mb-2 ml-3">ADVANCED MODE</span>
        <input type="checkbox" v-model="advancedMode" class="mx-2">

        <span class="font-semibold mb-2 ml-3">MAINTENANCE MODE</span>
        <input type="checkbox" v-model="maintenanceMode" class="mx-2">

        <span class="font-semibold mb-2 ml-3">TOOLTIPS</span>
        <input type="checkbox" v-model="tooltipMode" class="mx-2">

        <span class=" font-semibold mb-2 ml-3">HOTLOADING</span>
        ...
        <div class="text-left mt-3 ml-4 flex">
          ...
          <StopIcon v-if="autoMode" class="h-6 w-6 -ml-1 -mt-1" aria-hidden="false" :class="counter%10==i ? 'text-green-600 dark:text-green-300' : 'text-gray-200'"/>
        </span>
        </div>
