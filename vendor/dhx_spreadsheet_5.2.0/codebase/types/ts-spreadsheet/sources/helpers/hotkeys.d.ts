import { ISpreadsheet, IBufferManager } from "../types";
import { IGrid } from "../../../ts-grid";
import { EditLine } from "../EditLine";
import { Store } from "../Store";
export declare function isPrintableKey(e: KeyboardEvent): boolean;
export declare function getHtmlData(spreadsheet: ISpreadsheet, store: Store): string;
export declare function getKeysHandlers(spreadsheet: ISpreadsheet, grid: IGrid, bufferManager: IBufferManager, editLine: EditLine, store: Store): {
    escape: () => void;
    delete: () => void;
    enter: () => void;
    f2: () => void;
    pageUp: (e: any) => void;
    pageDown: (e: any) => void;
    "ctrl+a": (e: any) => void;
    "shift+arrowLeft": () => void;
    "shift+arrowRight": () => void;
    "shift+arrowDown": () => void;
    "shift+arrowUp": () => void;
    "ctrl+enter": () => void;
    "shift+enter": () => void;
    tab: (e: any) => void;
    "shift+tab": (e: any) => void;
    arrowLeft: (e: any) => void;
    arrowRight: (e: any) => void;
    arrowDown: (e: any) => void;
    arrowUp: (e: any) => void;
    "ctrl+arrowLeft": () => void;
    "ctrl+arrowRight": () => void;
    "ctrl+arrowDown": () => void;
    "ctrl+arrowUp": () => void;
    home: (e: any) => void;
    end: (e: any) => void;
    "ctrl+home": () => void;
    "ctrl+end": () => void;
    "ctrl+z": (e: any) => void;
    "ctrl+y": (e: any) => void;
    "ctrl+shift+z": () => void;
    "ctrl+b": () => void;
    "ctrl+i": () => void;
    "ctrl+u": (e: any) => void;
    "ctrl+f": (e: any) => void;
    "alt+shift+5": (e: any) => void;
    "ctrl+shift+x": (e: any) => void;
    "ctrl+shift+e": (e: any) => void;
    "ctrl+shift+l": (e: any) => void;
    "ctrl+shift+r": (e: any) => void;
    "shift + F11": (e: any) => void;
    "alt + arrowUp": (e: any) => void;
    "alt + arrowDown": (e: any) => void;
    "ctrl + space": (e: any) => void;
    "shift + space": (e: any) => void;
    "ctrl+k": (e: any) => void;
    backspace: () => void;
};
