...
class ii {
  constructor(n, e, i) {
    this._secondIteration = !1, this._gantt = n, this._constraintsHelper = i, this._graphHelper = e, this._asapStrategy = je.Create(n), this._alapStrategy = ze.Create(n), this._secondIterationRequired = !1;
  }
  generatePlan(n, e) {
...
// Lieuwe 05.05.25 : Needs refactor! Sandbox --> ResourcePlanning module. + unitTest 
class panaderoResourcePlanning {
    ...
    async getAssignments(resourceId) {
        var assignments;
        var store = await gantt.getDatastore(gantt.config.resource_store);    //resource
   ...
   // markers
    ...
    async configLayout (_resource_columns) {
        gantt.config.layout = {
            css: "gantt_container",
            rows: [
                {
    ...
    // plugins
    async fullScreenPlugin () { gantt.plugins({ fullscreen: true }); }
    async markerPlugin () { gantt.plugins({ marker: true }); }
    async groupingTasksPlugin () { gantt.plugins({grouping: true }); }

    // actions
    async refresh() { gantt.refreshData(); }

    async init(_dark) {
       var that = this;
        gantt.init("resource_id");
        gantt.setSkin(_dark ? "dark" : 'meadow');
        gantt.message({ text: "Panadero Resource Planning Module " + moduleVersion + " Initialize", expire: 0 });
    ...
    async reset(_dark) {
        ...
        await gantt.setSkin(_dark ? "dark" : 'meadow');
        await that.config();
        await that.configColumns();
        await that.init();
    }

    async fullscreen() {
        if (!gantt.getState().fullscreen) {
            // expanding the plan to full screen
            await gantt.expand();
        } else {
            // collapsing the plan to the normal mode
            await gantt.collapse();
        }
    }
    
    async parseResourcesStore(_resources) {
        ...
        resourcesStore.parse(_resources);
    }

    async parse (_data, _links) {
        gantt.parse({ data: _data, links: _links });
    }


    async loadZoomConfig () {
     this.zoomConfig = {
       levels: [{ name:"day", scale_height: 27, min_column_width:80, scales:[{unit: "day", step: 1, format: "%d %M"}]},
                ...
                { name:"month", scale_height: 50, min_column_width:120, scales:[
                               {unit: "month", format: "%F, %Y"},
                               {unit: "week", format: "Week #%W"}
                            ]},
                { name:"quarter", height: 50, min_column_width:90, scales:[
                ...
                { name:"year", scale_height: 50, min_column_width: 30, scales:[
...
function Ut(t) {
  ...
  for (e.sort(function(r, o) {
    ...
    var a = e[i];
    (li(a) || ci(a) || di(a)) && oi(a) || (e.splice(i, 1), i--);
  }
  return e;
}
function En() {
...
const Nn = Object.freeze(Object.defineProperty({ __proto__: null, addClassName: xt, closest: ut, getActiveElement: De, getChildNodes: An, getClassName: nt, getClosestSizedElement: Mn, getFocusableNodes: Ut, getNodePosition: J, getRelativeEventPosition: ct, getRelativeNodePosition: In, getRootNode: Dt, getScrollSize: En, getTargetNode: At, hasClass: function(t, n) {
  return "classList" in t ? t.classList.contains(n) : new RegExp("\\b" + n + "\\b").test(t.className);
}, hasShadowParent: function(t) {
  return !!Dt(t);
}, insertNode: Cn, isChildOf: tt, isShadowDomSupported: Ln, locateAttribute: Q, locateClassName: yt, removeClassName: Nt, removeNode: Dn, toNode: Fe }, Symbol.toStringTag, { value: "Module" })), lt = typeof window < "u" ? window : global;
let ui = class {
  constructor(t) {
...
function gt(t) {
  ...
  t.attachEvent = function(e, i, a) {
    e = "ev_" + e.toLowerCase(), n.listeners[e] || (n.listeners[e] = hi(this)), a && a.thisObject && (i = i.bind(a.thisObject));
...
const On = Object.freeze(Object.defineProperty({ __proto__: null, bind: z, copy: K, defined: G, event: Rn, eventRemove: Hn, mixin: R, uid: ht }, Symbol.toStringTag, { value: "Module" }));
function We(t, n) {
  t = t || Rn, n = n || Hn;
...
class mi {
  ...
  show(n, e) {
    ...
    if (tt(r, a) || (this.hide(), r.style.top = r.style.top || "0px", r.style.left = r.style.left || "0px", a.appendChild(r)), this._isLikeMouseEvent(n)) {
...
class vi {
  constructor(n) {
    this._listeners = {}, this.tooltip = new mi(n), this._gantt = n, this._domEvents = We(), this._initDelayedFunctions();
  }
  destructor() {
  ...
  attach(n) {
    ...
    const r = (o) => {
      ...
      if (tt(s, this.tooltip.getNode())) return;
      const d = () => {
...
const bi = { auto_scheduling: function(t) {
  Ke(t), Xe(t);
  var n = en(t), e = tn(), i = Be.Create(t), a = new ii(t, e, i), r = new ai(t, n), o = new ri(t, e, n);
  ...
  t.attachEvent("onParse", function() {
    ...
    if (t.callEvent("onBeforeAutoSchedule", [l]) !== !1) {
      ...
      if (h.length) t.callEvent("onAutoScheduleCircularLink", [h]);
        ...
        (function(f, k) {
          if (t.config.auto_scheduling_compatibility) for (var m = 0; m < k.length; m++) {
            var b = k[m], g = t.getTask(b.target);
  ...
  function e() {
    ...
    t.ext.clickDrag && t.ext.clickDrag.destructor(), t.ext.clickDrag = new ui(t);
    ...
    const r = t.$ui.getView("timeline"), o = new _i(i, t, r);
  ...
  var n = function(i) {
    var a = en(i), r = tn(), o = { _freeSlack: {}, _totalSlack: {}, _slackNeedCalculate: !0, _linkedTasksById: {}, _successorsByTaskId: {}, _projectEnd: null, _calculateSlacks: function() {
      ...
      s.forEach((f) => {
        ...
        if (!k) return;
        _[f.source] = !0;
        let m = u.getDist(k, i.getTask(f.target));
        m -= f.lag || 0, c[f.source] !== void 0 ? c[f.source] = Math.min(m, c[f.source]) : c[f.source] = m;
      });
      for (const f in h) {
        if (_[f]) continue;
        const k = h[f];
      ...
      for (var c = {}, h = i.getTaskByTime(), _ = 0; _ < l.length; _++) {
        ...
        if (this._isCompletedTask(k)) c[k.id] = 0;
          ...
          const m = d[k.id].linked;
          let b = null;
          for (var f = 0; f < m.length; f++) {
            const g = m[f], v = i.getTask(g.target);
            let p = 0;
            c[v.id] !== void 0 && (p += c[v.id]), p += u.getDist(k, v), p -= g.link.lag || 0, b = b === null ? p : Math.min(b, p);
          }
          c[k.id] = b || 0;
    ...
    n.config.keyboard_navigation = !0, n.config.keyboard_navigation_cells = !1, n.$keyboardNavigation = {}, n._compose = function() {
      ...
      e.$keyboardNavigation.TaskRow = function(i) {
        ...
        if (!i) {
          ...
          a.top < o.y || a.top + r > o.y + l ? e.scrollTo(null, a.top - 20) : e.config.scroll_on_click && d && (a.left > o.x + s ? e.scrollTo(a.left - e.config.task_scroll_offset) : a.left + a.width < o.x && e.scrollTo(a.left + a.width - e.config.task_scroll_offset));
        }
      ...
      e.$keyboardNavigation.TaskCell = function(i, a) {
        ...
        var a = e.locate(i);
        if (e.isTaskExists(a)) {
          var r = 0, o = Q(i, "data-column-index");
      ...
      var i = function(m) {
        if (n.config.keyboard_navigation && (n.config.keyboard_navigation_cells || !o(m)) && !s(m) && !function(b) {
          return !!ut(b.target, ".gantt_cal_light");
        }(m)) return e.keyDownHandler(m);
      }, a = function(m) {
        if (e.$preventDefault) return m.preventDefault(), n.$container.blur(), !1;
        e.awaitsFocus() || e.focusGlobalNode();
      }, r = function() {
        if (!e.isEnabled()) return;
        const m = !tt(document.activeElement, n.$container) && document.activeElement.localName != "body";
        var b = e.getActiveNode();
        if (b && !m) {
          var g, v, p = b.getNode();
          p && p.parentNode && (g = p.parentNode.scrollTop, v = p.parentNode.scrollLeft), b.focus(!0), p && p.parentNode && (p.parentNode.scrollTop = g, p.parentNode.scrollLeft = v);
        }
      };
      function o(m) {
        return !!ut(m.target, ".gantt_grid_editor_placeholder");
      }
      function s(m) {
        return !!ut(m.target, ".no_keyboard_navigation");
      }
      function l(m) {
        if (!n.config.keyboard_navigation || !n.config.keyboard_navigation_cells && o(m)) return !0;
        if (!s(m)) {
          var b, g = e.fromDomElement(m);
          if (g && (e.activeNode instanceof n.$keyboardNavigation.TaskCell && tt(m.target, n.$task) && (g = new n.$keyboardNavigation.TaskCell(g.taskId, e.activeNode.columnIndex)), b = g, n.config.show_grid && n.$ui.getView("grid") && n.config.keyboard_navigation_cells)) {
            const v = m.target.classList.contains("gantt_row"), p = m.target.closest(".gantt_task_line"), y = n.utils.dom.getNodePosition(n.$grid).x, w = y + n.$grid.offsetWidth, $ = n.utils.dom.getNodePosition(document.activeElement).x;
            if (v || p && ($ < y || w < $)) {
              let x = n.$grid.scrollLeft;
              const S = x + n.$grid.offsetWidth;
              let T = 0;
              v && (x = n.utils.dom.getRelativeEventPosition(m, n.$grid).x);
              for (let E = 0; E < n.config.columns.length; E++) {
                const C = n.config.columns[E];
                if (!C.hide && (T += C.width, x < T)) {
      ...
      var d = n.attachEvent("onGanttReady", function() {
        if (n.detachEvent(d), n.$data.tasksStore.attachEvent("onStoreUpdated", function(b) {
          if (n.config.keyboard_navigation && e.isEnabled()) {
            const g = e.getActiveNode(), v = n.$ui.getView("grid");
            if (!v || !v.$grid_data) return;
            const p = v.getItemTop(b), y = v.$grid_data.scrollTop, w = y + v.$grid_data.getBoundingClientRect().height;
          ...
          n._smart_render._redrawTasks = function(b, g) {
            if (n.config.keyboard_navigation && e.isEnabled()) {
              ...
              if (v && v.taskId !== void 0) {
                for (var p = !1, y = 0; y < g.length; y++) if (g[y].id == v.taskId && g[y].start_date) {
                  p = !0;
                  break;
                }
                p || g.push(n.getTask(v.taskId));
              }
            }
      ...
      n.attachEvent("onTaskCreated", function(m) {
        ...
        if (e.isEnabled()) {
          ...
          var g = 0, v = e.activeNode;
          v instanceof n.$keyboardNavigation.TaskCell && (g = v.columnIndex);
          var p = n.config.keyboard_navigation_cells ? n.$keyboardNavigation.TaskCell : n.$keyboardNavigation.TaskRow;
          b.type == n.config.types.placeholder && n.config.placeholder_task.focusOnCreate === !1 || e.setActiveNode(new p(m, g));
        }
      }), n.attachEvent("onTaskIdChange", function(m, b) {
        if (!n.config.keyboard_navigation) return !0;
        var g = e.activeNode;
        return e.isTaskFocused(m) && (g.taskId = b), !0;
      });
      ...
      function f(m) {
        for (var b = n.getGridColumns(), g = 0; g < b.length; g++) if (b[g].name == m) return g;
        return 0;
      }
      ...
      var k = {};
      gt(k), n.mixin(k, { addShortcut: function(m, b, g) {
        var v = _(g);
        v && v.prototype.bind(m, b);
      }, getShortcutHandler: function(m, b) {
        var g = n.$keyboardNavigation.shortcuts.parse(m);
        if (g.length) return k.getCommandHandler(g[0], b);
      }, getCommandHandler: function(m, b) {
        var g = _(b);
        if (g && m) return g.prototype.findHandler(m);
      }, removeShortcut: function(m, b) {
        var g = _(b);
        g && g.prototype.unbind(m);
      }, focus: function(m) {
        var b, g = m ? m.type : null, v = _(g);
        switch (g) {
          case "taskCell":
            b = new v(m.id, f(m.column));
            break;
          case "taskRow":
            b = new v(m.id);
            break;
        ...
        if (e.isEnabled()) {
          var m = e.getActiveNode(), b = (v = m) instanceof n.$keyboardNavigation.GanttNode ? "gantt" : v instanceof n.$keyboardNavigation.HeaderCell ? "headerCell" : v instanceof n.$keyboardNavigation.TaskRow ? "taskRow" : v instanceof n.$keyboardNavigation.TaskCell ? "taskCell" : null, g = n.getGridColumns();
          switch (b) {
            case "taskCell":
              return { type: "taskCell", id: m.taskId, column: g[m.columnIndex].name };
            case "taskRow":
              return { type: "taskRow", id: m.taskId };
            case "headerCell":
              return { type: "headerCell", column: g[m.index].name };
          }
        }
        var v;
        return null;
  ...
  t.ext || (t.ext = {}), t.ext.quickInfo = new fi(t), t.config.quickinfo_buttons = ["icon_edit", "icon_delete"], t.config.quick_info_detached = !0, t.config.show_quick_info = !0, t.templates.quick_info_title = function(a, r, o) {
  ...
  function s(u, c, h) {
    o(u.value, c, h), o(u.oldValue, c, h);
  }
  function l(u, c, h) {
    const _ = n;
    for (let f = 0; f < u.length; f++) {
      const k = u[f];
      for (let m = 0; m < k.commands.length; m++) k.commands[m].entity === _.command.entity.task ? r(k.commands[m], c, h) : k.commands[m].entity === _.command.entity.link && s(k.commands[m], c, h);
    }
  }
  function d(u, c, h) {
    const _ = n;
    for (let f = 0; f < u.length; f++) {
      const k = u[f];
      for (let m = 0; m < k.commands.length; m++) {
        const b = k.commands[m];
  ...
  function n(s, l, d) {
    ...
    for (let c = 0; c < s.length; c++) {
      ...
      }).reduce(function(f, k, m) {
        return k === h && f.push(m), f;
      }, []);
  ...
  t._groups = { relation_property: null, relation_id_property: "$group_id", group_id: null, group_text: null, loading: !1, loaded: 0, dynamicGroups: !1, set_relation_value: void 0, _searchCache: null, init: function(s) {
    ...
    this._searchCache = null, s.attachEvent("onBeforeTaskMove", function(c, h, _) {
      ...
      var m = s.getTask(c);
      if (this._groups.save_tree_structure && s.isTaskExists(m.parent) && s.isTaskExists(h)) {
        var b = s.getTask(m.parent), g = s.getTask(h);
        g.$virtual && s.isChildOf(b.id, g.id) && (m.parent = s.config.root_id);
        let v = !1, p = g;
        for (; p; ) c == p.parent && (v = !0), p = s.isTaskExists(p.parent) ? s.getTask(p.parent) : null;
        if (v) return !1;
      }
      return !0;
      ...
      if (s.config.order_branch && s.config.order_branch != "marker") {
        ...
        if (_.$initial_parent) {
          if (_.parent == s.config.root_id) {
            var f = s.getTask(_.$rendered_parent), k = s.getTask(_.$initial_parent), m = !1;
            this._groups.dynamicGroups && f[this._groups.group_id] != k[this._groups.group_id] && (m = !0), this._groups.dynamicGroups || f[this._groups.group_id] == k[this._groups.relation_property] || (m = !0), m && (_.parent = _.$initial_parent);
          }
    ...
    s.$data.tasksStore.setParent = function(c, h) {
      ...
      if (l.set_relation_value instanceof Function && s.isTaskExists(h)) {
        var _ = (m = s.getTask(h))[l.relation_id_property];
        if (!m.$virtual) {
          var f = r(m, l.relation_property);
        ...
        var m = s.getTask(h);
        l.dynamicGroups || (m.$virtual ? c[l.relation_property] = m[l.relation_id_property] : c[l.relation_property] = m[l.relation_property]), this._setParentInner.apply(this, arguments);
      } else l.dynamicGroups && (c[l.group_id] === void 0 || !c.$virtual && c[l.relation_property][0] === [][0]) && (c[l.relation_property] !== l.group_id ? c[l.relation_property] = c[l.relation_property] || 0 : c[l.relation_property] = 0);
    ...
    var u = function(f, k) {
      for (var m = !1, b = !1, g = 0; g < f.length; g++) {
        var v = f[g][k];
        if (Array.isArray(v) && (b = !0, v.length && v[0].resource_id !== void 0)) {
          m = !0;
          break;
        }
      }
      return { haveArrays: b, haveResourceAssignments: m };
    ...
    this._groups.regroup = function() {
      var f = t.getTaskByTime(), k = {}, m = !1;
      f.forEach(function(g) {
        g.$virtual && g.$open !== void 0 && (k[g[h]] = g.$open, m = !0);
      });
      var b = function(g, v, p) {
        var y;
        return y = g.groups ? p._groups.dynamicGroups ? function(w, $) {
          var x = {}, S = [], T = {}, E = $.relation_property, C = $.delimiter || ",", D = !1, M = 0;
          _t($.groups, function(N) {
            N.default && (D = !0, M = N.group_id), T[N.key || N[$.group_id]] = N;
          });
          for (var A = 0; A < w.length; A++) {
            var I, L, P = w[A][E];
            if (Lt(P)) if (P.length > 0) I = i(P), L = P.map(function(N, O) {
              var B;
              return B = N && typeof N == "object" ? N.resource_id : N, (N = T[B]).label || N.text;
            }).sort(), L = [...new Set(L)].join(C);
            else {
              if (D) continue;
              I = 0, L = $.default_group_label;
            }
            ...
            I !== void 0 && x[I] === void 0 && (x[I] = { key: I, label: L }, I === M && (x[I].default = !0), x[I][$.group_text] = L, x[I][$.group_id] = I);
          }
          return (S = function(N) {
            var O = [];
            for (var B in N) N.hasOwnProperty(B) && O.push(N[B]);
            return O;
      ...
      return b && m && b.forEach(function(g) {
    ...
    for (let o = 0; o < i.length; o++) {
      ...
      for (let h = 0; h < u.trace_x.length; h++) {
        ...
        if (n.cellColors) {
          ...
          if (k) {
            const m = k(_);
            m && (f.styles = t.ext.export_api._getStyles(m));
          }
        }
        d.push(f), s = l;
      }
...
} }, $i = { KEY_CODES: { UP: 38, DOWN: 40, LEFT: 37, RIGHT: 39, SPACE: 32, ENTER: 13, DELETE: 46, ESC: 27, TAB: 9 } };
class xi {
  constructor(n) {
...
var ot = function(t) {
  return this.pull = {}, this.$initItem = t.initItem, this.visibleOrder = at.$create(), this.fullOrder = at.$create(), this._skip_refresh = !1, this._filterRule = null, this._searchVisibleOrder = {}, this._indexRangeCache = {}, this._getItemsCache = null, this.$config = t, gt(this), this._attachDataChange(function() {
...
ot.prototype = { _attachDataChange: function(t) {
  ...
  this.$destroyed || (this.silent(function() {
    this.unselect();
  }), this.pull = {}, this.visibleOrder = at.$create(), this.fullOrder = at.$create(), this.isSilent() || (this.callEvent("onClearAll", []), this.refresh()));
}, silent: function(t, n) {
  var e = !1;
...
Ve.prototype = R({ _buildTree: function(t) {
  ...
  this.fullOrder = at.$create(), this._traverseBranches(function(n) {
  ...
  if (!ft(t, e)) {
    ...
    if (n == -1 && (n = s.length + 1), ft(o, e) && this.getBranchIndex(t) == n) return;
    if (this.callEvent("onBeforeItemMove", [t, e, n]) === !1) return !1;
    for (var l = [], d = 0; d < s.length; d++) Gt(s[d], null, this, this._ganttConfig) && (l.push(s[d]), s.splice(d, 1), d--);
    this._replace_branch_child(o, t);
    var u = (s = this.getChildren(e))[n];
    (u = dt(u, a)) ? s = s.slice(0, n).concat([t]).concat(s.slice(n)) : s.push(t), l.length && (s = s.concat(l)), ft(r.$rendered_parent, o) || ft(o, e) || (r.$rendered_parent = o), this.setParent(r, e), this._branches[e] = s;
  ...
  if (!this.exists(t)) return at.$create();
  ...
  this.hasChild(i) || (this._branches[i] = at.$create());
  ...
  var i = typeof t == "string" ? function(l, d) {
    return l[t] == d[t] || it(l[t]) && it(d[t]) && l[t].valueOf() == d[t].valueOf() ? 0 : l[t] > d[t] ? 1 : -1;
  } : t;
  if (n) {
    var a = i;
    i = function(l, d) {
...
const Ii = { create: function() {
  var t = R({}, { createDatastore: function(n) {
    var e = (n.type || "").toLowerCase() == "treedatastore" ? Ve : ot;
    if (n) {
      var i = this;
    ...
    var n;
    Y(this) || (n = this.getScrollState()), this.callEvent("onBeforeDataRender", []);
    for (var e = fe.call(this), i = 0; i < e.length; i++) e[i].refresh();
    this.config.preserve_scroll && !Y(this) && (n.x || n.y) && this.scrollTo(n.x, n.y), this.callEvent("onDataRender", []);
...
function Mi(t) {
  var n = function(u) {
    var c = new zn(u).primaryScale(), h = c.unit, _ = c.step;
    if (u.config.scale_offset_minimal) {
      var f = new jn(u), k = [f.primaryScale()].concat(f.getSubScales());
...
const Ie = { getSubtreeLinks: function(t, n) {
  var e = {};
...
class Li {
  ...
  attach() {
    ...
    this._dataProcessorHandlers.push(e.attachEvent("onAfterTaskAdd", function(s, l) {
      ...
      return e.config.cascade_delete && (i[s] = { tasks: Ie.getSubtreeTasks(e, s), links: Ie.getSubtreeLinks(e, s) }), !n.deleteAfterConfirmation || (n.setGanttMode("tasks"), n.setUpdated(s, !0, "deleted"), !1);
...
let ee = se, cn = class {
  ...
  ignore(t, n) {
    this._silent_mode = !0, t.call(n || lt), this._silent_mode = !1;
  }
  setUpdated(t, n, e) {
  ...
  _sendData(t, n) {
    ...
    if (this._tMode === "CUSTOM") {
      const l = this.getState(n), d = this.getActionByState(l), u = this.getGanttMode(), c = (_) => {
        let f = l || "updated", k = n, m = n;
        _ && (f = _.action || l, k = _.sid || k, m = _.id || _.tid || m), this.afterUpdateCallback(k, m, f, _, u);
      };
      let h;
...
See https://docs.dhtmlx.com/gantt/desktop__server_side.html#customrouting and https://docs.dhtmlx.com/gantt/api__gantt_createdataprocessor.html for details.`;
  ...
  _prepareArray(t, n) {
    return n.push(t), t.map((e) => it(e) ? this._prepareDate(e) : Array.isArray(e) && !Yt(n, e) ? this._prepareArray(e, n) : e && typeof e == "object" && !Yt(n, e) ? this._prepareObject(e, n) : e);
  }
  _prepareObject(t, n) {
    const e = {};
    n.push(t);
    for (const i in t) {
      ...
      const a = t[i];
      it(a) ? e[i] = this._prepareDate(a) : a === null ? e[i] = "" : Array.isArray(a) && !Yt(n, a) ? e[i] = this._prepareArray(a, n) : a && typeof a == "object" && !Yt(n, a) ? e[i] = this._prepareObject(a, n) : e[i] = a;
    }
    return e;
  }
  _prepareDataItem(t) {
...
const Ri = function(t) {
  return { _needRecalc: !0, reset: function() {
    ...
    let e = n.split("."), i = t.config.root_id;
    ...
    t.eachTask(function(e) {
      if (e.type == t.config.types.placeholder) return;
      if (n) return n = !1, void this._setWBSCode(e, "1");
      const i = this._getPrevNonPlaceholderSibling(e.id);
      if (i !== null) this._increaseWBS(e, i);
      else {
        let a = t.getParent(e.id);
        this._setWBSCode(e, t.getTask(a).$wbs + ".1");
      }
    }, t.config.root_id, this), this._needRecalc = !1;
  }, _increaseWBS: function(n, e) {
    let i = t.getTask(e).$wbs;
    i && (i = i.split("."), i[i.length - 1]++, this._setWBSCode(n, i.join(".")));
  }, _getPrevNonPlaceholderSibling: function(n) {
    let e, i = n;
    do
      e = t.getPrevSibling(i), i = e;
    while (e !== null && t.getTask(e).type == t.config.types.placeholder);
    return e;
  } };
};
function Hi(t) {
  ...
  function o(l, d, u) {
    ...
    return _t(d, function(f) {
      ...
      l in f && _t(Lt(f[l]) ? f[l] : [f[l]], function(k) {
        var m = k && k.resource_id ? k.resource_id : k;
        if (_[a(m)]) c.push(f);
        else if (!e) {
          var b = r(k, l);
          n[b] || (n[b] = []), n[b].push(f);
    ...
    const r = { renderEditableLabel: function(o, s, l, d, u) {
      ...
      a.config.resource_render_empty_cells = !0, function() {
        ...
        a.attachEvent("onGanttReady", function() {
          ...
          a.$layout.getCellsByType("viewCell").forEach(function(u) {
            u.$config && u.$config.view === "resourceTimeline" && u.$content && u.$content.attachEvent("onScroll", d);
          });
        });
      }();
      let o = null;
      ...
      a.attachEvent("onGanttReady", function() {
        ...
        a.event(a.$container, "keypress", function(d) {
          var u = d.target.closest(".resourceTimeline_cell [data-assignment-cell]");
          u && (d.keyCode !== 13 && d.keyCode !== 27 || u.blur());
        }), a.event(a.$container, "keydown", function(d) {
          d.key === "Tab" && (o = Ut(a.$container).indexOf(document.activeElement));
          ...
          if (!l) {
            ...
            var u = d.target.closest(".resourceTimeline_cell [data-assignment-cell]");
            if (u) {
              var c = (u.innerText || "").trim();
              c == "-" && (c = "0");
              var h = Number(c), _ = u.getAttribute("data-row-id"), f = u.getAttribute("data-assignment-id"), k = u.getAttribute("data-task"), m = u.getAttribute("data-resource-id"), b = a.templates.parse_date(u.getAttribute("data-start-date")), g = a.templates.parse_date(u.getAttribute("data-end-date")), v = a.getDatastore(a.config.resource_assignment_store);
              if (isNaN(h)) a.getDatastore(a.config.resource_store).refresh(_);
              else {
                var p = a.getTask(k);
                if (a.plugins().undo && a.ext.undo.saveState(k, "task"), f) {
                  if (h === (w = v.getItem(f)).value) return;
                  if (w.start_date.valueOf() === b.valueOf() && w.end_date.valueOf() === g.valueOf()) w.value = h, h ? v.updateItem(w.id) : v.removeItem(w.id);
                  else {
                    if (w.end_date.valueOf() > g.valueOf()) {
                      var y = a.copy(w);
                      y.id = a.uid(), y.start_date = g, y.duration = a.calculateDuration({ start_date: y.start_date, end_date: y.end_date, task: p }), y.delay = a.calculateDuration({ start_date: p.start_date, end_date: y.start_date, task: p }), y.mode = w.mode || "default", y.duration !== 0 && v.addItem(y);
                    }
                    w.start_date.valueOf() < b.valueOf() ? (w.end_date = b, w.duration = a.calculateDuration({ start_date: w.start_date, end_date: w.end_date, task: p }), w.mode = "fixedDuration", w.duration === 0 ? v.removeItem(w.id) : v.updateItem(w.id)) : v.removeItem(w.id), h && v.addItem({ task_id: w.task_id, resource_id: w.resource_id, value: h, start_date: b, end_date: g, duration: a.calculateDuration({ start_date: b, end_date: g, task: p }), delay: a.calculateDuration({ start_date: p.start_date, end_date: b, task: p }), mode: "fixedDuration" });
                  }
                  a.updateTaskAssignments(p.id), a.updateTask(p.id), s();
                } else if (h) {
                  var w = { task_id: k, resource_id: m, value: h, start_date: b, end_date: g, duration: a.calculateDuration({ start_date: b, end_date: g, task: p }), delay: a.calculateDuration({ start_date: p.start_date, end_date: b, task: p }), mode: "fixedDuration" };
                  v.addItem(w), a.updateTaskAssignments(p.id), a.updateTask(p.id), s();
                }
              }
            }
          }
...
function zi(t) {
  ...
  function r(f, k) {
    ...
    var m = Number(f.delay), b = !1;
    if (isNaN(m) ? (f.delay = 0, b = !0) : f.delay = m, t.defined(f.value) || (f.value = null), !f.task_id || !f.resource_id) return !1;
  ...
  function s(f, k) {
    ...
    var m, b, g = f.delay ? t.calculateEndDate({ start_date: k.start_date, duration: f.delay, task: k }) : new Date(k.start_date);
    return f.mode === a.fixedDuration ? (m = t.calculateEndDate({ start_date: g, duration: f.duration, task: k }), b = f.duration) : (m = new Date(k.end_date), b = k.duration - f.delay), { start_date: g, end_date: m, duration: b };
  }
  function l(f) {
    ...
    let g = i === e.auto;
    if (t.defined(m) && m) {
      Array.isArray(m) || (m = [m], g && (i = e.singleValue, g = !1));
      const v = {};
      m.forEach(function(p) {
        ...
        let y, w = a.default;
        p.mode || (p.start_date && p.end_date || p.start_date && p.duration) && (w = a.fixedDuration), y = p.id || !p.$id || v[p.$id] ? p.id && !v[p.id] ? p.id : t.uid() : p.$id, v[y] = !0;
        const $ = { id: y, start_date: p.start_date, duration: p.duration, end_date: p.end_date, delay: p.delay, task_id: f.id, resource_id: p.resource_id, value: p.value, mode: p.mode || w };
  ...
  function u(f, k) {
    k.sort(function(m, b) {
      return m.start_date && b.start_date && m.start_date.valueOf() != b.start_date.valueOf() ? m.start_date - b.start_date : 0;
    }), i == e.assignmentsArray ? f[t.config.resource_property] = k : i == e.resourceValueArray && (f[t.config.resource_property] = k.map(function(m) {
      return { $id: m.id, start_date: m.start_date, duration: m.duration, end_date: m.end_date, delay: m.delay, resource_id: m.resource_id, value: m.value, mode: m.mode };
    })), f[n] = k;
  }
  function c(f) {
    var k = l(f);
  ...
  function h(f, k) {
    var m = function(b, g) {
      var v = { inBoth: [], inTaskNotInStore: [], inStoreNotInTask: [] };
      if (i == e.singleValue) {
        var p = b[0], y = p ? p.resource_id : null, w = !1;
        g.forEach(function(T) {
          T.resource_id != y ? v.inStoreNotInTask.push(T) : T.resource_id == y && (v.inBoth.push({ store: T, task: p }), w = !0);
        }), !w && p && v.inTaskNotInStore.push(p);
        ...
        b.forEach(function(T) {
          ...
          if (!S[T.resource_id]) {
            S[T.resource_id] = !0;
            var E = $[T.resource_id], C = x[T.resource_id];
            E && C ? v.inBoth.push({ store: C, task: E }) : E && !C ? v.inTaskNotInStore.push(E) : !E && C && v.inStoreNotInTask.push(C);
          }
        });
        ...
        if (!S[E]) {
          S[E] = !0;
          var C = $[E], D = x[E];
          C && D ? v.inBoth.push({ store: D, task: C }) : C && !D ? v.inTaskNotInStore.push(C) : !C && D && v.inStoreNotInTask.push(D);
        }
      }));
      return v;
    }(l(f), k);
    m.inStoreNotInTask.forEach(function(b) {
      o.removeItem(b.id);
    }), m.inTaskNotInStore.forEach(function(b) {
      o.addItem(b);
    }), m.inBoth.forEach(function(b) {
      if (function(v, p) {
        var y = { id: !0 };
        for (var w in v) if (!y[w] && String(v[w]) !== String(p[w])) return !0;
        return !1;
      }(b.task, b.store)) (function(v, p) {
        var y = { id: !0 };
        for (var w in v) y[w] || (p[w] = v[w]);
  ...
  function _(f) {
    var k = f[n] || o.find(function(m) {
      return m.task_id == f.id;
    });
    h(f, k);
  }
  t.$data.assignmentsStore = o, t.attachEvent("onGanttReady", function() {
    if (t.config.process_resource_assignments) {
      ...
      t.attachEvent("onBeforeBatchUpdate", function() {
        ...
        if (k) {
          var y = {};
          for (var w in m) y[w] = t.getTaskAssignments(m[w].id);
          for (var w in t.config.process_resource_assignments && i === "resourceValueArray" && (p = null), m) h(m[w], y[w]);
        }
        k = !1, f = !1, m = {};
        ...
        return g = null, v = null, p = null, o.clearAll(), !0;
      }), t.attachEvent("onTaskIdChange", function(y, w) {
        o.find(function($) {
          return $.task_id == y;
        }).forEach(function($) {
          $.task_id = w, o.updateItem($.id);
        }), d(w);
      ...
      var g = null, v = null, p = null;
      o.attachEvent("onStoreUpdated", function() {
        return f && !b || (g = null, v = null, p = null), !0;
      }), t.getResourceAssignments = function(y, w) {
        var $ = t.defined(w) && w !== null;
        return g === null && (g = {}, v = {}, o.eachItem(function(x) {
          g[x.resource_id] || (g[x.resource_id] = []), g[x.resource_id].push(x);
          var S = x.resource_id + "-" + x.task_id;
          v[S] || (v[S] = []), v[S].push(x);
        })), $ ? (v[y + "-" + w] || []).slice() : (g[y] || []).slice();
      }, t.getTaskAssignments = function(y) {
        if (p === null) {
          var w = [];
          p = {}, o.eachItem(function($) {
            p[$.task_id] || (p[$.task_id] = []), p[$.task_id].push($), $.task_id == y && w.push($);
          });
        }
        return (p[y] || []).slice();
      }, t.getTaskResources = function(y) {
        const w = t.getDatastore("resource"), $ = t.getTaskAssignments(y), x = {};
        $.forEach(function(T) {
          x[T.resource_id] || (x[T.resource_id] = T.resource_id);
        });
        const S = [];
        for (const T in x) {
          const E = w.getItem(x[T]);
          E && S.push(E);
        }
        return S;
...
function Wi(t) {
  t.ext.formatters = { durationFormatter: function(n) {
    return n || (n = {}), n.store || (n.store = t.config.duration_unit), n.enter || (n.enter = t.config.duration_unit), Me.create(n, t);
...
const Ue = function(t, n) {
...
}, Fn = function(t, n, e, i, a, r) {
  const o = Ue(t, a), s = ne(t, n, a);
  ...
  if (s.splitChild && (h = Math.floor((l - d) / 2)), r || o) {
    let _ = Math.min(e.height, l) - d, f = 2, k = o && a.bar_height >= a.row_height, m = s.splitChild && e.height >= l;
    (k || m) && (f = 0), h = Math.floor(_ / 2) + f, e.rowHeight;
  }
  ...
  t.$data.baselineStore = n, t.adjustTaskHeightForBaselines = function(a) {
    ...
    if (d && t.config.show_chart) switch (t.config.baselines.render_mode) {
      case "taskRow":
        ...
        break;
      case "separateRow":
        r = d.getBarHeight(a.id), s ? (a.bar_height = a.bar_height || r, a.bar_height > r && (r = a.bar_height), a.row_height = r + l) : a.bar_height && (a.row_height = a.bar_height + 4), i(a);
        break;
      case "individualRow":
        r = d.getBarHeight(a.id), s ? (a.bar_height = a.bar_height || r, a.bar_height > r && (r = a.bar_height), o = l * s, a.row_height = r + o + 2) : a.bar_height && (a.row_height = a.bar_height + 4), i(a);
    }
  }, t.attachEvent("onGanttReady", function() {
    t.config.baselines && (t.attachEvent("onParse", function() {
      n.eachItem(function(a) {
        ...
        if (t.isTaskExists(r)) {
          ...
          s && o.baselines.push(a), Y(t) || (mt(o) ? e(o) : t.adjustTaskHeightForBaselines(o));
        }
      });
      ...
      return function(o) {
        ...
        }), s && (mt(o) ? e(o) : t.adjustTaskHeightForBaselines(o), t.render());
      }(r), !0;
      ...
      if (Ue && r.parent && t.isTaskExists(r.parent)) {
...
var ue = function(t, n, e, i) {
  this.$config = R({}, n || {}), this.$gantt = i, this.$parent = t, gt(this), this.$state = {}, R(this, Wn(this));
};
function Yi(t) {
  function n(e) {
  ...
  t.load = function() {
    ...
    var i = {};
    for (var a in e) a.charAt(0) != "$" && (i[a] = e[a], it(i[a]) && (i[a] = t.defined(t.templates.xml_format) ? t.templates.xml_format(i[a]) : t.templates.format_date(i[a])));
    return i;
  }, serialize: function() {
    var e = [], i = [];
    let a = [];
...
ue.prototype = { init: function(t) {
  ...
  var l = function(u, c) {
    var h = { column_before_start: u.bind(function(_, f, k) {
      var m = c.$getConfig(), b = Q(k, m.grid_resizer_column_attribute);
      if (!b || !ut(b, ".gantt_grid_column_resize_wrap")) return !1;
      var g = this.locate(k, m.grid_resizer_column_attribute), v = c.getGridColumns()[g];
      return c.callEvent("onColumnResizeStart", [g, v]) !== !1 && void 0;
    }, u), column_after_start: u.bind(function(_, f, k) {
      var m = c.$getConfig(), b = this.locate(k, m.grid_resizer_column_attribute);
      ...
      var m = c.$getConfig(), b = _.config, g = c.getGridColumns(), v = parseInt(b.drag_index, 10), p = g[v], y = J(c.$grid_scale), w = parseInt(b.marker.style.left, 10), $ = p.min_width ? p.min_width : m.min_grid_column_width, x = c.$grid_data.offsetWidth, S = 0, T = 0;
      m.rtl ? w = y.x + y.width - 1 - w : w -= y.x - 1;
      for (var E = 0; E < v; E++) $ += g[E].width, S += g[E].width;
      if (w < $ && (w = $), m.keep_grid_width) {
        var C = 0;
        for (E = v + 1; E < g.length; E++) g[E].min_width ? x -= g[E].min_width : m.min_grid_column_width && (x -= m.min_grid_column_width), g[E].max_width && C !== !1 ? C += g[E].max_width : C = !1;
        C && ($ = c.$grid_data.offsetWidth - C), w < $ && (w = $), w > x && (w = x);
      } else if (!c.$config.scrollable) {
        var D = w, M = u.$container.offsetWidth, A = 0;
        if (c.$grid_data.offsetWidth <= M - 25) for (E = v + 1; E < g.length; E++) A += g[E].width;
        else {
          for (E = v + 1; E >= 0; E--) A += g[E].width;
          A = M - A;
        }
        A > M && (A -= M);
        ...
        if (I && I.$config.mode == "y") {
          var L = I.$lastSize.x;
          M = Math.min(M, L - (I.$cells.length - 1));
        }
        D + A > M && (w = M - A);
      }
      return b.left = w - 1, T = Math.abs(w - S), p.max_width && T > p.max_width && (T = p.max_width), m.rtl && (S = y.width - S + 2 - T), b.marker.style.top = y.y + "px", b.marker.style.left = y.x - 1 + S + "px", b.marker.style.width = T + "px", c.callEvent("onColumnResize", [v, g[v], T - 1]), !0;
    }, u), column_drag_end: u.bind(function(_, f, k) {
      for (var m = c.$getConfig(), b = c.getGridColumns(), g = 0, v = parseInt(_.config.drag_index, 10), p = b[v], y = 0; y < v; y++) g += b[y].width;
      var w = p.min_width && _.config.left - g < p.min_width ? p.min_width : _.config.left - g;
      if (p.max_width && p.max_width < w && (w = p.max_width), c.callEvent("onColumnResizeEnd", [v, p, w]) !== !1 && p.width != w) {
        if (p.width = w, m.keep_grid_width) g = m.grid_width;
        else {
          y = v;
          for (var $ = b.length; y < $; y++) g += b[y].width;
        }
        c.callEvent("onColumnResizeComplete", [b, c._setColumnsWidth(g, v)]), c.$config.scrollable || u.$layout._syncCellSizes(c.$config.group, { value: m.grid_width, isGravity: !1 }), this.render();
      }
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), f = c.$getConfig(), k = new _(c.$grid_scale, { updates_per_second: 60 });
      u.defined(f.dnd_sensitivity) && (k.config.sensitivity = f.dnd_sensitivity), k.attachEvent("onBeforeDragStart", function(m, b) {
        return h.column_before_start(k, m, b);
      }), k.attachEvent("onAfterDragStart", function(m, b) {
        return h.column_after_start(k, m, b);
      }), k.attachEvent("onDragMove", function(m, b) {
        return h.column_drag_move(k, m, b);
      }), k.attachEvent("onDragEnd", function(m, b) {
        return h.column_drag_end(k, m, b);
      });
    }, doOnRender: function() {
      for (var _ = c.getGridColumns(), f = c.$getConfig(), k = 0, m = c.$config.width, b = f.scale_height, g = 0; g < _.length; g++) {
        var v, p = _[g];
        if (k += p.width, v = f.rtl ? m - k : k, p.resize && g != _.length - 1) {
          var y = document.createElement("div");
          y.className = "gantt_grid_column_resize_wrap", y.style.top = "0px", y.style.height = b + "px", y.innerHTML = "<div class='gantt_grid_column_resize'></div>", y.setAttribute(f.grid_resizer_column_attribute, g), y.setAttribute("column_index", g), u._waiAria.gridSeparatorAttr(y), c.$grid_scale.appendChild(y), y.style.left = Math.max(0, v) + "px";
        }
      }
    } };
  }(n, this);
  l.init(), this._renderHeaderResizers = l.doOnRender, this._mouseDelegates = Ge(n);
  var d = function(u, c) {
    var h = { row_before_start: u.bind(function(_, f, k) {
      var m = c.$getConfig(), b = c.$config.rowStore;
      if (!Q(k, m.task_grid_row_resizer_attribute)) return !1;
      var g = this.locate(k, m.task_grid_row_resizer_attribute), v = b.getItem(g);
      return c.callEvent("onBeforeRowResize", [v]) !== !1 && void 0;
    }, u), row_after_start: u.bind(function(_, f, k) {
      var m = c.$getConfig(), b = this.locate(k, m.task_grid_row_resizer_attribute);
      ...
      var m = c.$config.rowStore, b = c.$getConfig(), g = _.config, v = g.drag_id, p = c.getItemHeight(v), y = c.getItemTop(v) - f.scrollTop, w = J(c.$grid_data), $ = parseInt(g.marker.style.top, 10), x = y + w.y, S = 0, T = b.min_task_grid_row_height;
      return (S = $ - x) < T && (S = T), g.marker.style.left = w.x + "px", g.marker.style.top = x - 1 + "px", g.marker.style.height = Math.abs(S) + 1 + "px", g.marker_height = S, c.callEvent("onRowResize", [v, m.getItem(v), S + p]), !0;
    }, u), row_drag_end: u.bind(function(_, f, k) {
      var m = c.$config.rowStore, b = _.config, g = b.drag_id, v = m.getItem(g), p = c.getItemHeight(g), y = b.marker_height;
      c.callEvent("onBeforeRowResizeEnd", [g, v, y]) !== !1 && v.row_height != y && (v.row_height = y, m.updateItem(g), c.callEvent("onAfterRowResize", [g, v, p, y]), this.render());
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), f = c.$getConfig(), k = new _(c.$grid_data, { updates_per_second: 60 });
      u.defined(f.dnd_sensitivity) && (k.config.sensitivity = f.dnd_sensitivity), k.attachEvent("onBeforeDragStart", function(m, b) {
        return h.row_before_start(k, m, b);
      }), k.attachEvent("onAfterDragStart", function(m, b) {
        return h.row_after_start(k, m, b);
      }), k.attachEvent("onDragMove", function(m, b) {
        return h.row_drag_move(k, m, b);
      }), k.attachEvent("onDragEnd", function(m, b) {
        return h.row_drag_end(k, m, b);
      });
    } };
  }(n, this);
  ...
  for (var r = [], o = 0, s = e.labels, l = n.scale_height - 1, d = 0; d < a.length; d++) {
    ...
    var f = t._sort && c.name == t._sort.name ? `<div data-column-id="${c.name}" class="gantt_sort gantt_${t._sort.direction}"></div>` : "", k = ["gantt_grid_head_cell", "gantt_grid_head_" + c.name, u ? "gantt_last_cell" : "", i.grid_header_class(c.name, c)].join(" "), m = "width:" + (h - (u ? 1 : 0)) + "px;", b = c.label || s["column_" + c.name] || s[c.name];
    b = b || "";
    var g = "<div class='" + k + "' style='" + m + "' " + t._waiAria.gridScaleCellAttrString(c, b) + " data-column-id='" + c.name + "' column_id='" + c.name + "' data-column-name='" + c.name + "' data-column-index='" + d + "'>" + b + f + "</div>";
    r.push(g);
  }
...
var Vn = function(t) {
  return { getWorkHoursArguments: function() {
    var n = arguments[0];
    if (!Et((n = it(n) ? { date: n } : R({}, n)).date)) throw t.assert(!1, "Invalid date argument for getWorkHours method"), new Error("Invalid date argument for getWorkHours method");
    return n;
    ...
    if (r = it(o) ? new ke(arguments[0], arguments[1], arguments[2], void 0, arguments[3], void 0, arguments[4]) : new ke(o.start_date, o.duration, o.unit, o.step, o.task, null, o.calendar), o.id !== null && o.id !== void 0 && (r.task = o, r.unit = null, r.step = null), r.unit = r.unit || t.config.duration_unit, r.step = r.step || t.config.duration_step, !Et(r.start_date)) throw t.assert(!1, "Invalid start_date argument for calculateEndDate method"), new Error("Invalid start_date argument for calculateEndDate method");
    return r;
  } };
};
function Un() {
}
Un.prototype = { _getIntervals: function(t) {
...
Pe.prototype = { units: ["year", "month", "week", "day", "hour", "minute"], _clearCaches: function() {
  ...
  for (a = z(e == "minute" ? this._getMinutesPerDay : this.getHoursPerDay, this); o.valueOf() < s.valueOf(); ) if (s - o > 27648e5 && o.getDate() === 0) {
  ...
  for (var s = 0, l = [], d = 0; a < n; ) {
    ...
    if (l.length && d) if (l[l.length - 1].end <= f && _ > d) a += d, i = this.$gantt.date.add(i, -1, "day");
      ...
      for (var k = !1, m = null, b = null, g = l.length - 1; g >= 0; g--) if (l[g].start < f - 1 && l[g].end >= f - 1) {
        k = !0, m = l[g], b = l[g - 1];
        break;
      }
      if (k) if (f === m.end && _ >= m.durationMinutes) a += m.durationMinutes, i = this.$gantt.date.add(i, -m.durationMinutes, "minute");
      else if (!r && _ <= f / 60 - m.startMinute) a += _, i = this.$gantt.date.add(i, -_, "minute");
      else if (r) _ <= f / 60 - m.startMinute ? (a += _, i = this.$gantt.date.add(i, -_, "minute")) : (a += f / 60 - m.startMinute, i = b ? new Date(i.getFullYear(), i.getMonth(), i.getDate(), 0, 0, b.end) : this.$gantt.date.day_start(i));
      else {
        var v = this._getMinutesPerHour(i);
        v <= _ ? (a += v, i = this._nextDate(i, "hour", e)) : (o = this._subtractMinutesUntilHourStart(i, _), a += o.added, i = o.end);
      }
  ...
  if (a < n) {
    var $ = n - a;
    o = this._subtractMinutesUntilHourStart(i, $), a += o.added, i = o.end;
  }
  return i;
  ...
  for (var o = 0, s = [], l = 0, d = this._isMinutePrecision(i); a < n; ) {
    ...
    if (s.length && l) if (s[0].start >= h && c >= l) {
      ...
      if (_) if (h === f.start && c >= f.durationMinutes) a += f.durationMinutes, i = this.$gantt.date.add(i, f.durationMinutes, "minute");
        ...
        m <= c ? (a += m, i = d ? this.$gantt.date.add(i, m, "minute") : this._nextDate(i, "hour", e)) : (a += (r = this._addMinutesUntilHourEnd(i, c)).added, i = r.end);
      }
...
function qn(t) {
  this.$gantt = t.$gantt, this.argumentsHelper = Vn(this.$gantt), this.calendarManager = t, this.$disabledCalendar = new Re(this.$gantt, this.argumentsHelper);
}
Gn.prototype = { _calendars: {}, _convertWorkTimeSettings: function(t) {
  var n = t.days;
  if (n && !t.dates) {
    t.dates = t.dates || {};
  ...
  var n = new Pe(this.$gantt, Vn(this.$gantt));
  n.id = String(ht());
  ...
  if (n.resource_calendars) {
    var e;
    if (e = this._legacyConfig === !1 ? n.resource_property : zt.getResourceProperty(n), Array.isArray(t[e])) n.dynamic_resource_calendars ? i = ta.getCalendarIdFromMultipleResources(t[e], this) : a = this.getResourceCalendar(t[e]);
...
} }, qn.prototype = { _getCalendar: function(t) {
  var n;
  ...
  t._init_task_timing = function(o) {
    ...
    if (o) {
      ...
      if (l || d.start_date && d.end_date && (c.$no_start || c.$no_end)) {
        const k = d.$auto_start_date ? "$auto_start_date" : "start_date", m = d.$auto_end_date ? "$auto_end_date" : "end_date";
        var _ = d[k].valueOf(), f = d[m].valueOf();
        t.resetProjectDates(d), l || _ != d[k].valueOf() || f != d[m].valueOf() || (h = !1), h && !s && this.refreshTask(d.id, !0), c.scheduled_summary && (h = !0);
      }
      h && u && this.isTaskExists(u) && this._update_parents(u, s, l);
    }
  }, t.roundDate = function(o) {
    var s = t.getScale();
    it(o) && (o = { date: o, unit: s ? s.unit : t.config.duration_unit, step: s ? s.step : t.config.duration_step });
...
function ia(t) {
  ...
  var i = (function() {
    ...
    Y(t) || (this.$root.innerHTML = "", this.$root.gantt = this, Ae(this), this.config.layout.id = "main", this.$layout = this.$ui.createView("layout", this.$root, this.config.layout), this.$layout.attachEvent("onBeforeResize", function() {
  ...
  t.resetLayout = function() {
    ...
    this.callEvent("onBeforeGanttReady", []), this._update_flags(), this.$services.getService("templateLoader").initTemplates(this), i(), this.$root = null, r && (this.$root = Fe(r), a(), this.$mouseEvents.reset(this.$root), function(o) {
...
https://docs.dhtmlx.com/gantt/faq.html#theganttchartisntrenderedcorrectly`);
    ...
    if (Ae(this), Y(t)) t.refreshData();
      ...
      if (this.config.preserve_scroll = !1, this.$layout.resize(), this.config.preserve_scroll = l, this.config.preserve_scroll && o) {
        ...
        this.$layout.getScrollbarsInfo().forEach((c) => {
          const h = t.$ui.getView(c.id), _ = t.utils.dom.isChildOf(h.$view, t.$container);
          c.boundViews.forEach((f) => {
            const k = t.$ui.getView(f);
            c.y && c.y != o.y && k && !_ && k.scrollTo(void 0, 0), c.x_pos && c.x_pos != o.x && k && _ && k.scrollTo(c.x_pos, void 0);
          });
        });
      }
    }
...
function Oa(t) {
  ...
  n.plugins = function(l) {
    ...
    return { cache: !0, method: "get", parse: function(d) {
      return typeof d != "string" ? d : (d = d.replace(/^[\s]+/, ""), typeof DOMParser > "u" || bt.isIE ? lt.ActiveXObject !== void 0 && ((u = new lt.ActiveXObject("Microsoft.XMLDOM")).async = "false", u.loadXML(d)) : u = new DOMParser().parseFromString(d, "text/xml"), u);
      var u;
      ...
      var h = It("GET", arguments);
      return this.query(h);
    }, getSync: function(d, u) {
      var c = It("GET", arguments);
      return c.async = !1, this.query(c);
    }, put: function(d, u, c, h) {
      var _ = It("PUT", arguments);
      return this.query(_);
    }, del: function(d, u, c) {
      var h = It("DELETE", arguments);
      return this.query(h);
    }, post: function(d, u, c, h) {
      arguments.length == 1 ? u = "" : arguments.length == 2 && typeof u == "function" && (c = u, u = "");
      var _ = It("POST", arguments);
      return this.query(_);
    }, postSync: function(d, u, c) {
      u = u === null ? "" : String(u);
      var h = It("POST", arguments);
      return h.async = !1, this.query(h);
    }, _call: function(d, u, c, h, _, f) {
      return new l.Promise(function(k, m) {
        var b = typeof XMLHttpRequest !== void 0 ? new XMLHttpRequest() : new lt.ActiveXObject("Microsoft.XMLHTTP"), g = navigator.userAgent.match(/AppleWebKit/) !== null && navigator.userAgent.match(/Qt/) !== null && navigator.userAgent.match(/Safari/) !== null;
        h && (b.onreadystatechange = function() {
          if (b.readyState == 4 || g && b.readyState == 3) {
            ...
            setTimeout(function() {
              typeof _ == "function" && _.apply(lt, [{ xmlDoc: b, filePath: u }]), k(b), typeof _ == "function" && (_ = null, b = null);
            }, 0);
          }
        });
        var v = !this || !this.cache;
        if (d == "GET" && v && (u += (u.indexOf("?") >= 0 ? "&" : "?") + "dhxr" + (/* @__PURE__ */ new Date()).getTime() + "=1"), b.open(d, u, h), f) for (var p in f) b.setRequestHeader(p, f[p]);
  ...
  var a = function(l) {
    ...
    function u(c, h) {
      ...
      this._drag_start_timer = null, l.attachEvent("onGanttScroll", z(function(m, b) {
        this.clearDragTimer();
      }, this));
      for (var f = { passive: !1 }, k = 0; k < _.length; k++) z(function(m) {
        l.event(c, m.down, z(function(g) {
          m.accessor(g) && (g.button !== void 0 && g.button !== 0 || (h.preventDefault && h.selector && ut(g.target, h.selector) && g.preventDefault(), l.config.touch && g.timeStamp && g.timeStamp - 0 < 300 || (this._settings.original_target = d(g), this._settings.original_element_sizes = { ...ct(g, Mn(c)), width: g.target.offsetWidth, height: g.target.offsetHeight }, l.config.touch ? (this.clearDragTimer(), this._drag_start_timer = setTimeout(z(function() {
            l.getState().lightbox || this.dragStart(c, g, m);
          }, this), l.config.touch_drag)) : this.dragStart(c, g, m))));
        }, this), f);
        var b = document.body;
        l.event(b, m.up, z(function(g) {
          m.accessor(g) && this.clearDragTimer();
        }, this), f);
      }, this)(_[k]);
    }
    return u.prototype = { traceDragEvents: function(c, h) {
      var _ = z(function(v) {
        return this.dragMove(c, v, h.accessor);
      }, this);
      z(function(v) {
        return this.dragScroll(c, v);
      }, this);
      var f = z(function(v) {
        if (!this.config.started || !G(this.config.updates_per_second) || Bn(this, this.config.updates_per_second)) {
          var p = _(v);
          if (p) try {
            v && v.preventDefault && v.cancelable && v.preventDefault();
          } catch {
          }
          return p;
        }
      }, this), k = Dt(l.$root), m = this.config.mousemoveContainer || Dt(l.$root), b = { passive: !1 }, g = z(function(v) {
        return l.eventRemove(m, h.move, f), l.eventRemove(k, h.up, g, b), this.dragEnd(c);
      }, this);
      l.event(m, h.move, f, b), l.event(k, h.up, g, b);
      ...
      if (c.push({ move: "mousemove", down: "mousedown", up: "mouseup", accessor: function(_) {
        ...
        } }) : lt.navigator.pointerEnabled ? c.push({ move: "pointermove", down: "pointerdown", up: "pointerup", accessor: function(_) {
          return _.pointerType == "mouse" ? null : _;
        } }) : lt.navigator.msPointerEnabled && c.push({ move: "MSPointerMove", down: "MSPointerDown", up: "MSPointerUp", accessor: function(_) {
  ...
  var r = /* @__PURE__ */ function(l) {
    ...
    return { initTemplates: function() {
      var c = l.date, h = c.date_to_str, _ = l.config, f = h(_.xml_date || _.date_format, _.server_utc), k = c.str_to_date(_.xml_date || _.date_format, _.server_utc);
      u("date_scale", !0, void 0, l.config, l.templates), u("date_grid", !0, "grid_date_format", l.config, l.templates), u("task_date", !0, void 0, l.config, l.templates), l.mixin(l.templates, { xml_format: void 0, format_date: f, xml_date: void 0, parse_date: k, progress_text: function(m, b, g) {
        return "";
      }, grid_header_class: function(m, b) {
        return "";
      }, task_text: function(m, b, g) {
        return g.text;
      }, task_class: function(m, b, g) {
        return "";
      }, task_end_date: function(m) {
        return l.templates.task_date(m);
      }, grid_row_class: function(m, b, g) {
        return "";
      }, task_row_class: function(m, b, g) {
        return "";
      ...
      }, date_grid: function(m, b, g) {
        return b && l.isUnscheduledTask(b) && l.config.show_unscheduled ? l.templates.task_unscheduled_time(b) : l.templates.grid_date_format(m, g);
      }, task_time: function(m, b, g) {
        return l.isUnscheduledTask(g) && l.config.show_unscheduled ? l.templates.task_unscheduled_time(g) : l.templates.task_date(m) + " - " + l.templates.task_end_date(b);
      }, task_unscheduled_time: function(m) {
        return "";
      }, time_picker: h(_.time_picker), link_class: function(m) {
        return "";
      }, link_description: function(m) {
        var b = l.getTask(m.source), g = l.getTask(m.target);
        return "<b>" + b.text + "</b> &ndash;  <b>" + g.text + "</b>";
      }, drag_link: function(m, b, g, v) {
        m = l.getTask(m);
        var p = l.locale.labels, y = "<b>" + m.text + "</b> " + (b ? p.link_start : p.link_end) + "<br/>";
        return g && (y += "<b> " + (g = l.getTask(g)).text + "</b> " + (v ? p.link_start : p.link_end) + "<br/>"), y;
      }, drag_link_class: function(m, b, g, v) {
        var p = "";
        return m && g && (p = " " + (l.isLinkAllowed(m, g, b, v) ? "gantt_link_allow" : "gantt_link_deny")), "gantt_link_tooltip" + p;
      }, tooltip_date_format: c.date_to_str("%Y-%m-%d"), tooltip_text: function(m, b, g) {
  ...
  o.registerProvider("global", function() {
    ...
    var u, c = l.createDatastore({ name: "task", type: "treeDatastore", rootId: function() {
      ...
      return l.config;
    } }), h = l.createDatastore({ name: "link", initItem: z(function(g) {
      return this.defined(g.id) || (g.id = this.uid()), g;
    }, l) });
    function _(g) {
      var v = l.isTaskVisible(g);
      if (!v && l.isTaskExists(g)) {
        var p = l.getParent(g);
        l.isTaskExists(p) && l.isTaskVisible(p) && (p = l.getTask(p), l.isSplitTask(p) && (v = !0));
      }
      return v;
    }
    function f(g) {
      if (l.isTaskExists(g.source)) {
        var v = l.getTask(g.source);
        v.$source = v.$source || [], v.$source.push(g.id);
      }
    ...
    function k(g) {
      if (l.isTaskExists(g.source)) {
        for (var v = l.getTask(g.source), p = 0; p < v.$source.length; p++) if (v.$source[p] == g.id) {
          v.$source.splice(p, 1);
          break;
        }
      }
    ...
    function m() {
      for (var g = null, v = l.$data.tasksStore.getItems(), p = 0, y = v.length; p < y; p++) (g = v[p]).$source = [], g.$target = [];
      var w = l.$data.linksStore.getItems();
      for (p = 0, y = w.length; p < y; p++) f(w[p]);
    }
    function b(g) {
      var v = g.source, p = g.target;
      for (var y in g.events) (function(w, $) {
        v.attachEvent(w, function() {
    ...
    l.attachEvent("onDestroy", function() {
      ...
      for (var v = l.getTask(g.source).$source, p = 0; p < v.length; p++) {
        var y = l.getLink(v[p]), w = g.source == y.source, $ = g.target == y.target, x = g.type == y.type;
        if (w && $ && x) return !1;
      }
      return !0;
    }), c.attachEvent("onBeforeRefreshAll", function() {
      if (!c._skipTaskRecalculation) for (var g = c.getVisibleItems(), v = 0; v < g.length; v++) {
        var p = g[v];
        p.$index = v, p.$local_index = l.getTaskIndex(p.id), l.resetProjectDates(p);
      }
    }), c.attachEvent("onFilterItem", function(g, v) {
      if (l.config.show_tasks_outside_timescale) return !0;
      var p = null, y = null;
      if (l.config.start_date && l.config.end_date) {
        if (l._isAllowedUnscheduledTask(v)) return !0;
        if (p = l.config.start_date.valueOf(), y = l.config.end_date.valueOf(), +v.start_date > y || +v.end_date < +p) return !1;
      }
      return !0;
    }), c.attachEvent("onIdChange", function(g, v) {
      l._update_flags(g, v);
      var p = l.getTask(v);
      ...
      var v = c.getItem(g);
      v.$source || (v.$source = []);
      for (var p = 0; p < v.$source.length; p++) h.refresh(v.$source[p]);
      for (v.$target || (v.$target = []), p = 0; p < v.$target.length; p++) h.refresh(v.$target[p]);
    }), c.attachEvent("onBeforeItemMove", function(g, v, p) {
      return !Gt(g, l, c) || (console.log("The placeholder task cannot be moved to another position."), !1);
    }), c.attachEvent("onAfterItemMove", function(g, v, p) {
      var y = l.getTask(g);
    ...
    }), c.attachEvent("onStoreUpdated", function(g, v, p) {
    ...
    }), h.attachEvent("onAfterAdd", function(g, v) {
      f(v);
    }), h.attachEvent("onAfterUpdate", function(g, v) {
      m();
    }), h.attachEvent("onAfterDelete", function(g, v) {
      k(v);
    }), h.attachEvent("onAfterSilentDelete", function(g, v) {
      k(v);
    }), h.attachEvent("onBeforeIdChange", function(g, v) {
      k(l.mixin({ id: g }, l.$data.linksStore.getItem(v))), f(l.$data.linksStore.getItem(v));
    }), h.attachEvent("onFilterItem", function(g, v) {
      if (!l.config.show_links) return !1;
      var p = _(v.source), y = _(v.target);
      return !(!p || !y || l._isAllowedUnscheduledTask(l.getTask(v.source)) || l._isAllowedUnscheduledTask(l.getTask(v.target))) && l.callEvent("onBeforeLinkDisplay", [g, v]);
    }), u = {}, l.attachEvent("onBeforeTaskDelete", function(g, v) {
      return u[g] = Ie.getSubtreeLinks(l, g), !0;
    }), l.attachEvent("onAfterTaskDelete", function(g, v) {
      u[g] && l.$data.linksStore.silent(function() {
        for (var p in u[g]) l.isLinkExists(p) && l.$data.linksStore.removeItem(p), k(u[g][p]);
        u[g] = null;
      });
    }), l.attachEvent("onAfterLinkDelete", function(g, v) {
      l.isTaskExists(v.source) && l.refreshTask(v.source), l.isTaskExists(v.target) && l.refreshTask(v.target);
    ...
    l._cached_functions = { cache: {}, mode: !1, critical_path_mode: !1, wrap_methods: function(c, h) {
      ...
      if (_) {
        ...
        h._prefetch_originals[c] = _, h[c] = function() {
          for (var k = new Array(arguments.length), m = 0, b = arguments.length; m < b; m++) k[m] = arguments[m];
          if (f.active) {
            var g = f.get_arguments_hash(Array.prototype.slice.call(k));
            f.cache[c] || (f.cache[c] = {});
            var v = f.cache[c];
            if (f.has_cached_value(v, g)) return f.get_cached_value(v, g);
            var p = _.apply(this, k);
            return f.cache_value(v, g, p), p;
          }
  ...
  var s = new wa({ en: ha, ar: aa, be: ra, ca: sa, cn: oa, cs: la, da, de: ca, el: ua, es: _a, fa: ga, fi: fa, fr: pa, he: ma, hr: va, hu: ka, id: ya, it: ba, jp: $a, kr: xa, nb: Sa, nl: Ta, no: Ea, pl: Ca, pt: Da, ro: Aa, ru: Ia, si: Ma, sk: La, sv: Na, tr: Pa, ua: Ra });
...
function Ba(t) {
  ...
  function i(g, v) {
    var p = g.callback;
    k.hide(g.box), e = g.box = null, p && p(v);
  }
  function a(g) {
    if (e) {
      var v = g.which || g.keyCode, p = !1;
      if (m.keyboard) {
        if (v == 13 || v == 32) {
          var y = g.target || g.srcElement;
          nt(y).indexOf("gantt_popup_button") > -1 && y.click ? y.click() : (i(e, !0), p = !0);
        }
        v == 27 && (i(e, !1), p = !0);
      }
  ...
  function s(g, v, p) {
    return "<div " + t._waiAria.messageButtonAttrString(g) + " class='gantt_popup_button " + ("gantt_" + v.toLowerCase().replace(/ /g, "_") + "_button") + "' data-result='" + p + "' result='" + p + "' ><div>" + g + "</div></div>";
  }
  function l() {
    for (var g = [].slice.apply(arguments, [0]), v = 0; v < g.length; v++) if (g[v]) return g[v];
  }
  function d(g, v, p) {
    var y = g.tagName ? g : function(x, S, T) {
      var E = document.createElement("div"), C = ht();
      t._waiAria.messageModalAttr(E, C), E.className = " gantt_modal_box gantt-" + x.type, E.setAttribute(n, 1);
      var D = "";
      if (x.width && (E.style.width = x.width), x.height && (E.style.height = x.height), x.title && (D += '<div class="gantt_popup_title">' + x.title + "</div>"), D += '<div class="gantt_popup_text" id="' + C + '"><span>' + (x.content ? "" : x.text) + '</span></div><div  class="gantt_popup_controls">', S && (D += s(l(x.ok, t.locale.labels.message_ok, "OK"), "ok", !0)), T && (D += s(l(x.cancel, t.locale.labels.message_cancel, "Cancel"), "cancel", !1)), x.buttons) for (var M = 0; M < x.buttons.length; M++) {
        var A = x.buttons[M];
        D += typeof A == "object" ? s(A.label, A.css || "gantt_" + A.label.toLowerCase() + "_button", A.value || M) : s(A, A, M);
      }
      if (D += "</div>", E.innerHTML = D, x.content) {
        var I = x.content;
        typeof I == "string" && (I = document.getElementById(I)), I.style.display == "none" && (I.style.display = ""), E.childNodes[x.title ? 1 : 0].appendChild(I);
      }
      return E.onclick = function(L) {
        ...
        if (P.className || (P = P.parentNode), ut(P, ".gantt_popup_button")) {
          ...
          i(x, N = N == "true" || N != "false" && N);
        }
      }, x.box = E, (S || T) && (e = x), E;
    }(g, v, p);
    g.hidden || o(!0), document.body.appendChild(y);
    var w = Math.abs(Math.floor(((window.innerWidth || document.documentElement.offsetWidth) - y.offsetWidth) / 2)), $ = Math.abs(Math.floor(((window.innerHeight || document.documentElement.offsetHeight) - y.offsetHeight) / 2));
    return g.position == "top" ? y.style.top = "-3px" : y.style.top = $ + "px", y.style.left = w + "px", y.onkeydown = a, k.focus(y), g.hidden && k.hide(y), t.callEvent("onMessagePopup", [y]), y;
  }
  function u(g) {
    return d(g, !0, !1);
  }
  ...
  function _(g, v, p) {
    return typeof g != "object" && (typeof v == "function" && (p = v, v = ""), g = { text: g, type: v, callback: p }), g;
  }
  function f(g, v, p, y) {
    return typeof g != "object" && (g = { text: g, type: v, expire: p, id: y }), g.id = g.id || ht(), g.expire = g.expire || m.expire, g;
  }
  ...
  k.hide = function(g) {
    ...
    setTimeout(function() {
      var v = Ut(g);
      v.length && v[0].focus && v[0].focus();
    }, 1);
  };
  var m = function(g, v, p, y) {
    switch ((g = f.apply(this, arguments)).type = g.type || "info", g.type.split("-")[0]) {
      ...
      default:
        return function(w) {
          m.area || (m.area = document.createElement("div"), m.area.className = "gantt_message_area", m.area.style[m.position] = "5px", document.body.appendChild(m.area)), m.hide(w.id);
          var $ = document.createElement("div");
          return $.innerHTML = "<div>" + w.text + "</div>", $.className = "gantt-info gantt-" + w.type, $.onclick = function() {
            m.hide(w.id), w = null;
          }, t._waiAria.messageInfoAttr($), m.position == "bottom" && m.area.firstChild ? m.area.insertBefore($, m.area.firstChild) : m.area.appendChild($), w.expire > 0 && (m.timers[w.id] = window.setTimeout(function() {
            m && m.hide(w.id);
          }, w.expire)), m.pull[w.id] = $, $ = null, w.id;
        }(g);
    }
  };
  m.seed = (/* @__PURE__ */ new Date()).valueOf(), m.uid = ht, m.expire = 4e3, m.keyboard = !0, m.position = "top", m.pull = {}, m.timers = {}, m.hideAll = function() {
    for (var g in m.pull) m.hide(g);
  }, m.hide = function(g) {
    var v = m.pull[g];
    v && v.parentNode && (window.setTimeout(function() {
      v.parentNode.removeChild(v), v = null;
    }, 2e3), v.className += " hidden", m.timers[g] && window.clearTimeout(m.timers[g]), delete m.pull[g]);
  };
  var b = [];
  return t.attachEvent("onMessagePopup", function(g) {
    b.push(g);
  }), t.attachEvent("onAfterMessagePopup", function(g) {
    for (var v = 0; v < b.length; v++) b[v] === g && (b.splice(v, 1), v--);
  }), t.attachEvent("onDestroy", function() {
...
const ja = function(t) {
  ...
  function i(a, r, o, s) {
    ...
    return l.configure && l.configure(d), za(d, s), d.$id || (d.$id = o.id || t.uid()), d.$parent || typeof r != "object" || (d.$parent = r), d.$config || (d.$config = o), e[d.$id] && (d.$id = t.uid()), e[d.$id] = d, d;
  }
...
var Fa = /* @__PURE__ */ function(t) {
  return function(n) {
    ...
    function r(h) {
      var _ = (h = h || window.event).target || h.srcElement, f = n.locate(_), k = n.locate(_, n.config.link_attribute), m = !n.checkEvent("onContextMenu") || n.callEvent("onContextMenu", [f, k, h]);
      return m || (h.preventDefault ? h.preventDefault() : h.returnValue = !1), m;
    }
    function o(h, _) {
      for (var f = h.target || h.srcElement, k = []; f; ) {
        var m = t.getClassName(f);
        if (m) {
          m = m.split(" ");
          for (var b = 0; b < m.length; b++) if (m[b] && _[m[b]]) for (var g = _[m[b]], v = 0; v < g.length; v++) g[v].root && !t.isChildOf(f, g[v].root) || k.push(g[v].handler);
        }
        f = f.parentNode;
      }
      return k;
    ...
    function s(h, _, f) {
      for (var k = !0, m = 0; m < h.length; m++) {
        var b = h[m].call(n, _, f, _.target || _.srcElement);
        k = k && !(b !== void 0 && b !== !0);
      }
      return k;
    }
    ...
    return { reset: c, global: function(h, _, f) {
      ...
      if (e[h] && e[h][_]) {
        for (var m = e[h], b = m[_], g = 0; g < b.length; g++) b[g].root == k && (b.splice(g, 1), g--);
        b.length || delete m[_];
      }
    }, callHandler: function(h, _, f, k) {
      var m = e[h][_];
      if (m) for (var b = 0; b < m.length; b++) (f || m[b].root) && m[b].root !== f || m[b].handler.apply(this, k);
...
var Va = function(t) {
  var n = /* @__PURE__ */ function(e) {
    ...
    function o(s, l, d) {
      ...
      var k = l.filter;
      return d && d.setAttribute(e.config.layer_attribute, !0), a[s] = { render_item: function(m, b, g, v, p) {
        if (b = b || d, !k || k(m)) {
          var y = v || r(l), w = p || (y ? y.$getConfig() : null), $ = g;
          !$ && w && w.smart_rendering && ($ = y.getViewPort());
          var x = null;
          !Wt(e) && (h || f) && $ ? (f ? f(m, $, y, w, e) : vn($, h(m, y, w, e))) && (x = u.call(e, m, y, w, $)) : x = u.call(e, m, y, w, $), this.append(m, x, b);
          var S = b.nodeType == 11;
          _ && !S && x && _.call(e, m, x, y);
        } else this.remove_item(m.id);
      }, clear: function(m) {
        this.rendered = i[s] = {}, l.append || this.clear_container(m);
      }, clear_container: function(m) {
        (m = m || d) && (m.innerHTML = "");
      }, get_visible_range: function(m) {
        var b, g, v = r(l), p = v ? v.$getConfig() : null;
        return p && p.smart_rendering && (b = v.getViewPort()), v && b && (typeof l.renderer == "function" ? g = Z(e, v, 0, m, b) : l.renderer && l.renderer.getVisibleRange && (g = l.renderer.getVisibleRange(e, v, p, m, b))), g || (g = { start: 0, end: m.count() }), g;
        ...
        this.clear(b);
        var v = null, p = r(l), y = p ? p.$getConfig() : null;
        y && y.smart_rendering && (v = p.getViewPort());
        for (var w = 0, $ = m.length; w < $; w++) this.render_item(m[w], g, v, p, y);
        b.appendChild(g, b);
        var x = {};
        ...
        if (_) {
          var T = {};
          for (var w in this.rendered) S[w] || (T[w] = this.rendered[w], _.call(e, x[w], this.rendered[w], p));
        }
      }, update_items: function(m, b) {
        var g = r(l), v = g ? g.$getConfig() : null;
        if (g && g.$getConfig().smart_rendering && !Wt(e) && this.rendered && (h || f)) {
          ...
          var $ = {}, x = {};
          for (var S in this.rendered) x[S] = !0, $[S] = !0;
          for (var T = {}, E = (S = 0, m.length); S < E; S++) {
            var C = m[S], D = this.rendered[C.id];
            x[C.id] = !1, D && D.parentNode ? (f ? f(C, y, g, v, e) : vn(y, h(C, g, v, e))) ? (c && c.call(e, C, D, g, v, y), this.restore(C, p)) : x[C.id] = !0 : (T[m[S].id] = !0, this.render_item(m[S], p, y, g, v));
          }
          for (var S in x) x[S] && this.hide(S);
          if (p.childNodes.length && b.appendChild(p, b), _) {
            var M = {};
            for (var S in this.rendered) $[S] && !T[S] || (M[S] = this.rendered[S], _.call(e, w[S], this.rendered[S], g));
          }
        }
      }, append: function(m, b, g) {
        this.rendered && (b ? (this.rendered[m.id] && this.rendered[m.id].parentNode ? this.replace_item(m.id, b) : g.appendChild(b), this.rendered[m.id] = b) : this.rendered[m.id] && this.remove_item(m.id));
      }, replace_item: function(m, b) {
        var g = this.rendered[m];
        g && g.parentNode && g.parentNode.replaceChild(b, g), this.rendered[m] = b;
        ...
        var g = this.rendered[m.id];
        g ? g.parentNode || this.append(m, g, b || d) : this.render_item(m, b || d);
  ...
  return { createGroup: function(e, i, a, r) {
    var o = { tempCollection: [], renderers: {}, container: e, filters: [], getLayers: function() {
      ...
      for (var l = this.container(), d = this.tempCollection, u = 0; u < d.length; u++) if (s = d[u], this.container() || s && s.container && tt(s.container, document.body)) {
  ...
  function i(r, o, s) {
    o.$getConfig(), r.getVisibleItems().forEach(function(l) {
      var d = function(u, c, h, _) {
        ...
        if (!f || !k) return null;
        var m = 100, b = Math.min(f.left, k.left) - m, g = Math.max(f.left + f.width, k.left + k.width) + m, v = Math.min(f.top, k.top) - m, p = Math.max(f.top + f.height, k.top + k.height) + m;
        return { top: v, height: p - v, bottom: p, left: b, width: g - b, right: g };
      }(l, o, 0, s);
...
function Jn(t, n, e, i, a) {
  ...
  return f > k && (m = k, k = f, f = m), h += -100, _ += c, f += -100, k += c, !(n.x > _ && n.x > k) && !(n.x_end < h && n.x_end < f);
}
function Ua(t, n) {
  if (t.view) {
    var e = t.view;
...
var wt = function() {
  function t(n, e, i, a) {
    n && (this.$container = Fe(n), this.$parent = n), this.$config = R(e, { headerHeight: 33 }), this.$gantt = a, this.$domEvents = a._createDomEventScope(), this.$id = e.id || "c" + ht(), this.$name = "cell", this.$factory = i, gt(this);
  }
...
var Kn = function(t) {
  ...
  return W(n, t), n.prototype.destructor = function() {
    ...
    return e.forEach((a) => {
      let r = {};
      const { visible: o, direction: s, size: l, scrollSize: d, position: u } = a.getScrollState();
      let c = a._getLinkedViews().map((h) => h.$config.id);
      r.id = a.$id, r.visible = o, r.boundViews = c, s === "x" ? (r.x = l, r.x_inner = d, r.x_pos = u || 0) : (r.y = l, r.y_inner = d, r.y_pos = u || 0), i.push(r);
    }), i;
  ...
  return W(e, t), e.prototype.init = function(a) {
    ...
    for (var a, r = 0, o = 0, s = this._isHorizontal(), l = this._getLinkedViews(), d = s ? "scrollWidth" : "scrollHeight", u = s ? "contentX" : "contentY", c = s ? "x" : "y", h = this._getScrollOffset(), _ = 0; _ < l.length; _++) if ((a = l[_]) && a.$content && a.$content.getSize && !a.$config.hidden) {
      ...
      if (f = k.hasOwnProperty(d) ? k[d] : k[u], h) k[u] > k[c] && k[u] > r && f > k[c] - h + 2 && (r = f + (s ? 0 : 2), o = k[c]);
        ...
        var m = Math.max(k[u] - f, 0);
        (f += m) > Math.max(k[c] - m, 0) && f > r && (r = f, o = k[c]);
      }
    }
    ...
    if (this._checkWheelTarget(r)) {
      ...
      if (k !== !1 && n.indexOf(k) >= 0 && (!a[k] || a.deltaX || a.wheelDeltaX || (_ = 2 * f, f = 0)), _ && Math.abs(_) > Math.abs(f)) {
        ...
        var m = _ / -40, b = this._oldLeft, g = b + 30 * m;
        ...
        m = f / -40, f === void 0 && (m = a.detail);
        var v = this._oldTop, p = this.$scroll_ver.scrollTop + 30 * m;
        if (this.scrollVertically(p), this.$scroll_ver.scrollTop = p, v == this.$scroll_ver.scrollTop) return !0;
  ...
  function l(c, h, _) {
    var f = Math.floor(500 / c) || 1, k = Math.floor(500 / h) || 1, m = document.createElement("canvas");
    m.height = h * k, m.width = c * f;
    var b = m.getContext("2d");
    return function(v, p, y, w, $, x) {
      var S = $.createImageData(p * w, v * y);
      S.imageSmoothingEnabled = !1;
      for (var T = 1 * x.rightBorderWidth, E = a(x.rightBorderColor), C = 0, D = 0, M = 0, A = 1; A <= w; A++) for (C = A * p - 1, M = 0; M < T; M++) for (D = 0; D < v * y; D++) g(C - M, D, E, S);
      var I = 1 * x.bottomBorderWidth, L = a(x.bottomBorderColor);
      D = 0;
      for (var P = 1; P <= y; P++) for (D = P * v - 1, M = 0; M < I; M++) for (C = 0; C < p * w; C++) g(C, D - M, L, S);
      $.putImageData(S, 0, 0);
    }(h, c, k, f, b, _), m.toDataURL();
    function g(v, p, y, w) {
      var $ = 4 * (p * (c * f) + v);
      w.data[$] = y.r, w.data[$ + 1] = y.g, w.data[$ + 2] = y.b, w.data[$ + 3] = y.a;
    }
  }
  function d(c) {
  ...
  return { render: function(c, h, _, f, k) {
    if ((h.static_background || h.timeline_placeholder) && document.createElement("canvas").getContext) {
      ...
      var m = function(p) {
        ...
        var $ = y.firstChild, x = getComputedStyle(y), S = getComputedStyle($), T = { bottomBorderWidth: x.getPropertyValue("border-bottom-width").replace("px", ""), rightBorderWidth: S.getPropertyValue("border-right-width").replace("px", ""), bottomBorderColor: x.getPropertyValue("border-bottom-color"), rightBorderColor: S.getPropertyValue("border-right-color") };
        ...
        for (var C in S) {
          ...
          if (!r(M)) {
            var A = l(D, T, p);
            s(M, A), E += "." + d(M) + "{ background-image: url('" + A + "');}";
          }
          x[C] = d(M);
        }
        return E && (function() {
      ...
      }(m, 0, _, k), g = function(p, y, w, $) {
        var x, S, T = [], E = 0, C = w.width.filter(function(F) {
          return !!F;
        }), D = 0, M = 1e5;
        if (n.isIE) {
          var A = navigator.appVersion || "";
          A.indexOf("Windows NT 6.2") == -1 && A.indexOf("Windows NT 6.1") == -1 && A.indexOf("Windows NT 6.0") == -1 || (M = 2e4);
        }
        for (var I = 0; I < C.length; I++) {
          var L = C[I];
          if (L != S && S !== void 0 || I == C.length - 1 || E > M) {
            for (var P = $, N = 0, O = Math.floor(M / y.row_height) * y.row_height, B = E; P > 0; ) {
              var H = Math.min(P, O);
              P -= O, (x = document.createElement("div")).style.height = H + "px", x.style.position = "absolute", x.style.top = N + "px", x.style.left = D + "px", x.style.pointerEvents = "none", x.style.whiteSpace = "no-wrap", x.className = p[S || L], I == C.length - 1 && (B = L + B - 1), x.style.width = B + "px", T.push(x), N += H;
            }
            E = 0, D += B;
          }
          L && (E += L, S = L);
        ...
        return T;
      }(b, h, _, f), v = document.createDocumentFragment();
...
var qe = function(t, n, e, i) {
  this.$config = R({}, n || {}), this.$scaleHelper = new jn(i), this.$gantt = i, this._posFromDateCache = {}, this._timelineDragScroll = null, R(this, Wn(this)), gt(this);
};
function ye(t) {
    ...
    a.$config.fetchTasks && a.silent(function() {
      ...
      for (var l in a.eachItem(function(d) {
        if (d.$role != "task") {
          ...
          u.sort(function(h, _) {
            const f = a.pull, k = f[`${h.task_id}_${h.resource_id}`], m = f[`${_.task_id}_${_.resource_id}`];
            return k && m ? k.$local_index - m.$local_index : 0;
          }), u.forEach(function(h) {
            if (!c[h.task_id] && i.isTaskExists(h.task_id)) {
              c[h.task_id] = !0;
              var _, f = i.getTask(h.task_id);
              (_ = a.$config.copyOnParse ? i.copy(f) : Object.create(f)).id = f.id + "_" + d.id, _.$task_id = f.id, _.$resource_id = d.id, _[a.$parentProperty] = d.id, _.$role = "task", r.push(_), o[_.id] = !0;
            }
          });
        } else s[d.id] = !0;
...
qe.prototype = { init: function(t) {
  ...
  var t = this, n = Xa(), e = t.$config.rowStore;
  ...
  var u = 0, c = t.count;
  !a.smart_scales || isNaN(n) || isNaN(e) || (u = Bt(t.left, n), c = Bt(t.left, e) + 1), l = t.css || function() {
  ...
  for (var h = u; h < c && t.trace_x[h]; h++) {
    ...
    t.height;
    var k = t.left[h], m = "", b = "", g = "";
    if (f) {
      m = "width:" + f + "px;" + (a.smart_scales ? "position:absolute;left:" + k + "px" : "");
      const p = this.getViewPort(), y = (a.scales[i] || {}).sticky;
      let w = "";
      const $ = 70;
      if (y !== !1 && f > $ || y === !0) {
        if (k < p.x && k + f / 2 - $ / 2 < p.x) w = ` style='position:absolute;left: ${p.x - k + 10}px;' `;
        else if (k + f / 2 + $ / 2 > p.x_end && f > $) {
          let x = p.x_end - k - 10, S = "-100%";
          x < $ && (x = $, S = `-${x}px`), w = ` style='position:absolute;left: ${x}px;transform: translate(${S},0);' `;
        }
      }
      g = "gantt_scale_cell" + (h == t.count - 1 ? " gantt_last_cell" : ""), (b = l.call(this, s)) && (g += " " + b);
      var v = `<div class='${g}' ${this.$gantt._waiAria.getTimelineCellAttr(_)} style='${m}'><span ${w}>${_}</span></div>`;
      o.push(v);
    }
  }
  return o.join("");
...
var Za = function(t) {
  ...
  return W(n, t), R(n.prototype, { init: function() {
    ...
    t.prototype._initEvents.apply(this, arguments), this._mouseDelegates.delegate("click", "gantt_row", e.bind(function(i, a, r) {
      ...
      if (!o) return !0;
      var s = Q(i, this.$config.item_attribute);
    ...
    switch (h) {
      ...
      default:
        if (t.isVisible()) _ = !0;
          ...
          var k = o.modifiers, m = k.alt || k.ctrl || k.meta || k.shift;
          k.alt || m && r.getCommandHandler(o, "taskCell") || d && !t.isVisible() && (e.startEdit(l.id, l.columnName), _ = !0);
        }
    }
    return !_;
  });
...
function sr(t) {
  ...
  function s(l, d, u) {
    var c = function(m, b, g) {
      var v = [];
      return [...new Set(b)].forEach(function(p) {
        var y = i(g).parse(p);
        y && (y.target = m, y.id = "predecessor_generated", t.isLinkAllowed(y) && (y.id = void 0, v.push(y)));
      }), v;
    }(l.id, d, u), h = {};
...
function dr(t) {
  t.config.editor_types = { text: new (nr())(), number: new (ir())(), select: new (ar())(), date: new (rr(t))(), predecessor: new (sr(t))(), duration: new (or(t))() };
}
function cr(t) {
  var n = /* @__PURE__ */ function(a) {
    ...
    return { setMapping: function(o) {
      ...
      return r || (a.config.keyboard_navigation_cells && a.ext.keyboardNavigation ? er : tr);
    } };
  }(t), e = {};
  gt(e);
  var i = { init: dr, createEditors: function(a) {
    function r(c, h) {
      var _ = a.$getConfig(), f = function(b, g) {
        for (var v = a.$getConfig(), p = a.getItemTop(b), y = a.getItemHeight(b), w = a.getGridColumns(), $ = 0, x = 0, S = 0, T = 0; T < w.length; T++) {
          if (w[T].name == g) {
            S = w[T].width;
            break;
          }
          v.rtl ? x += w[T].width : $ += w[T].width;
        }
        return v.rtl ? { top: p, right: x, height: y, width: S } : { top: p, left: $, height: y, width: S };
      ...
      var m = function(b, g) {
        for (var v = b.getGridColumns(), p = 0; p < v.length; p++) if (v[p].name == g) return p;
        return 0;
      }(a, h);
      return k.setAttribute("data-column-index", m), t._waiAria.inlineEditorAttr(k), _.rtl ? k.style.cssText = ["top:" + f.top + "px", "right:" + f.right + "px", "width:" + f.width + "px", "height:" + f.height + "px"].join(";") : k.style.cssText = ["top:" + f.top + "px", "left:" + f.left + "px", "width:" + f.width + "px", "height:" + f.height + "px"].join(";"), k;
    }
    var o = lr(t), s = [], l = [], d = null, u = { _itemId: null, _columnName: null, _editor: null, _editorType: null, _placeholder: null, locateCell: function(c) {
      if (!tt(c, a.$grid)) return null;
      var h = Q(c, a.$config.item_attribute), _ = Q(c, "data-column-name");
      if (h && _) {
      ...
      if (k) {
        var m = a.$getConfig().editor_types[k.type], b = r(_.id, _.columnName);
        a.$grid_data.appendChild(b), m.show(_.id, f, k, b), this._editor = m, this._placeholder = b, this._itemId = _.id, this._columnName = _.columnName, this._editorType = k.type;
        var g = n.getMapping();
        g.onShow && g.onShow(this, b, a);
      }
    }, setValue: function() {
      var c = this.getState(), h = c.id, _ = c.columnName, f = a.getColumn(_), k = d.getItem(h), m = this.getEditorConfig(_);
      if (m) {
        var b = k[m.map_to];
        m.map_to == "auto" && (b = d.getItem(h)), this._editor.set_value(b, h, f, this._placeholder), this.focus();
      }
    }, focus: function() {
      ...
      if (this.isVisible() && d.exists(this._itemId) && this.isChanged()) {
        ...
        if (d.exists(c)) {
          ...
          if (this.callEvent("onBeforeSave", [k]) !== !1 && (!this._editor.is_valid || this._editor.is_valid(k.newValue, k.id, a.getColumn(h), this._placeholder))) {
            var m = f.map_to, b = k.newValue;
            m != "auto" ? (_[m] = b, o(_, m, t.config.inline_editors_date_processing), d.updateItem(c)) : this._editor.save(c, a.getColumn(h), this._placeholder), this.callEvent("onSave", [k]);
          }
          this.hide();
        }
      } else this.hide();
...
function be(t) {
  function n(r, o, s) {
    ...
    const k = t.config.baselines && r.baselines && r.baselines.length, m = t.config.baselines && (t.config.baselines.render_mode == "separateRow" || t.config.baselines.render_mode == "individualRow");
    if (k && m && r.bar_height !== "full" && r.bar_height < r.row_height) if (c === d.types.milestone) {
    ...
    o.$config.item_attribute && (b.setAttribute(o.$config.item_attribute, r.id), b.setAttribute(o.$config.bind + "_id", r.id)), d.show_progress && c != d.types.milestone && function(T, E, C, D, M) {
      var A = 1 * T.progress || 0;
      C = Math.max(C - 2, 0);
      var I = document.createElement("div"), L = Math.round(C * A);
      L = Math.min(C, L), I.style.width = L + "px", I.className = "gantt_task_progress", I.innerHTML = M.progress_text(T.start_date, T.end_date, T), D.rtl && (I.style.position = "absolute", I.style.right = "0px");
      var P = document.createElement("div");
      P.className = "gantt_task_progress_wrapper", P.appendChild(I), E.appendChild(P);
      const N = !t.isReadonly(T), O = t.ext.dragTimeline && t.ext.dragTimeline._isDragInProgress();
      if (t.config.drag_progress && (N || O)) {
...
<path d="M5.58397 1.52543C5.78189 1.22856 6.21811 1.22856 6.41602 1.52543L10.5475 7.72265C10.769 8.05493 10.5308 8.5 10.1315 8.5L1.86852 8.5C1.46917 8.5 1.23097 8.05493 1.45249 7.72265L5.58397 1.52543Z" fill="var(--dhx-gantt-progress-handle-background)" stroke="var(--dhx-gantt-progress-handle-border)"/>
</svg>`, I.appendChild(B), E.appendChild(B);
      }
    }(r, b, g, d, u);
    var v = function(T, E, C) {
    ...
    var p = function(T, E, C, D) {
      ...
      if (t.getTaskType(L.type) == M.types.milestone ? A.push("gantt_milestone") : t.getTaskType(L.type) == M.types.project && A.push("gantt_project"), A.push("gantt_bar_" + t.getTaskType(L.type)), t.isSummaryTask(L) && A.push("gantt_dependent_task"), t.isSplitTask(L) && (M.open_split_tasks && !L.$open || !M.open_split_tasks) && A.push("gantt_split_parent"), M.select_task && t.isSelectedTask(C) && A.push("gantt_selected"), C == I.drag_id && (A.push("gantt_drag_" + I.drag_mode), I.touch_drag && A.push("gantt_touch_" + I.drag_mode)), I.link_source_id == C && (A.push("gantt_link_source"), I.link_from_start ? A.push("gantt_link_from_start") : A.push("gantt_link_from_end")), I.link_target_id == C && A.push("gantt_link_target"), M.highlight_critical_path && t.isCriticalTask && t.isCriticalTask(L) && A.push("gantt_critical_task"), I.link_landing_area && I.link_target_id && I.link_source_id && I.link_target_id != I.link_source_id && (I.link_target_id == C || I.link_source_id == C)) {
        var P = I.link_source_id, N = I.link_from_start, O = I.link_to_start, B = "";
        B = t.isLinkAllowed(P, C, N, O) ? O ? "link_start_allow" : "link_finish_allow" : O ? "link_start_deny" : "link_finish_deny", A.push(B);
      }
      return A.join(" ");
    ...
    var w = function(T, E, C, D) {
      ...
      return D && (A = { type: "marginRight", value: D }), e(T, C.leftside_text, M, A);
    }(r, d, u, _);
    w && b.appendChild(w), w = function(T, E, C, D) {
      var M = "gantt_right " + i(!!E.rtl, T), A = null;
      return D && (A = { type: "marginLeft", value: D }), e(T, C.rightside_text, M, A);
    }(r, d, u, _), w && b.appendChild(w), t._waiAria.setTaskBarAttr(r, b);
    var $ = t.getState();
    const x = !t.isReadonly(r), S = t.ext.dragTimeline && t.ext.dragTimeline._isDragInProgress();
    return (x || S) && (d.drag_resize && !t.isSummaryTask(r) && c != d.types.milestone && a(b, "gantt_task_drag", r, function(T) {
      var E = document.createElement("div");
...
function hr(t) {
  function n(r, o, s) {
    ...
    v.classList.add("gantt_task_line_planned", "gantt_task_line", "gantt_project");
    const p = o.getItemPosition(r, r.start_date, r.end_date);
    v.style.cssText = ["position:absolute", "left:" + p.left + "px", "top:" + (f / 2 + 1) + "px", "height:5px", "width:" + p.width + "px"].join(";"), v.style.setProperty("--dhx-gantt-scheduled-summary-bracket-size", "10px"), b.appendChild(v);
    const y = document.createElement("div"), w = o.getItemPosition(r, r.$auto_start_date || r.start_date, r.$auto_end_date || r.end_date);
    y.classList.add("gantt_task_line_actual", "gantt_task_line", "gantt_project"), y.style.cssText = ["position:absolute", "left:" + w.left + "px", "top:16px", "height:8px", "width:" + w.width + "px"].join(";"), b.appendChild(y), d.show_progress && c != d.types.milestone && function(T, E, C, D, M) {
      var A = 1 * T.progress || 0;
      C = Math.max(C - 2, 0);
      var I = document.createElement("div"), L = Math.round(C * A);
      L = Math.min(C, L), T.progressColor && (I.style.backgroundColor = T.progressColor, I.style.opacity = 1), I.style.width = L + "px", I.className = "gantt_task_progress", I.innerHTML = M.progress_text(T.start_date, T.end_date, T), D.rtl && (I.style.position = "absolute", I.style.right = "0px");
      var P = document.createElement("div");
      if (P.className = "gantt_task_progress_wrapper", P.appendChild(I), E.appendChild(P), t.config.drag_progress && !t.isReadonly(T)) {
        var N = document.createElement("div"), O = L;
        D.rtl && (O = C - L), N.style.left = O + "px", N.className = "gantt_task_progress_drag", I.appendChild(N), E.appendChild(N);
      }
    }(r, y, g, d, u);
    var $ = function(T, E, C, D) {
      ...
      E && A.push(E);
      var I = t.getState(), L = t.getTask(C);
      if (t.getTaskType(L.type) == M.types.milestone ? A.push("gantt_milestone") : t.getTaskType(L.type) == M.types.project && A.push("gantt_project"), A.push("gantt_bar_" + t.getTaskType(L.type)), t.isSummaryTask(L) && A.push("gantt_dependent_task"), t.isSplitTask(L) && (M.open_split_tasks && !L.$open || !M.open_split_tasks) && A.push("gantt_split_parent"), M.select_task && t.isSelectedTask(C) && A.push("gantt_selected"), C == I.drag_id && (A.push("gantt_drag_" + I.drag_mode), I.touch_drag && A.push("gantt_touch_" + I.drag_mode)), I.link_source_id == C && A.push("gantt_link_source"), I.link_target_id == C && A.push("gantt_link_target"), M.highlight_critical_path && t.isCriticalTask && t.isCriticalTask(L) && A.push("gantt_critical_task"), I.link_landing_area && I.link_target_id && I.link_source_id && I.link_target_id != I.link_source_id && (I.link_target_id == C || I.link_source_id == C)) {
        var P = I.link_source_id, N = I.link_from_start, O = I.link_to_start, B = "";
        B = t.isLinkAllowed(P, C, N, O) ? O ? "link_start_allow" : "link_finish_allow" : O ? "link_start_deny" : "link_finish_deny", A.push(B);
      }
      return A.join(" ");
    }("gantt_task_line", u.task_class(r.$auto_start_date || r.start_date, r.$auto_end_date || r.end_date, r), r.id, o);
    (r.color || r.progressColor || r.textColor) && ($ += " gantt_task_inline_color"), l.width < 20 && ($ += " gantt_thin_task"), (r.start_date > r.$auto_start_date || r.end_date < r.$auto_end_date) && ($ += " gantt_project_scheduling_conflict"), b.className = $, b.style.top = f + l.top + "px", b.style.height = (c == d.types.milestone ? l.height : h) + "px", r.color && b.style.setProperty("--dhx-gantt-task-background", r.color), r.textColor && b.style.setProperty("--dhx-gantt-task-color", r.textColor), r.progressColor && b.style.setProperty("--dhx-gantt-task-progress-color", r.progressColor);
    var x = function(T, E, C, D) {
      var M = "gantt_left " + i(!E.rtl, T), A = null;
      return D && (A = { type: "marginRight", value: D }), e(T, C.leftside_text, M, A);
    }(r, d, u, _);
    x && v.appendChild(x), x = function(T, E, C, D) {
      var M = "gantt_right " + i(!!E.rtl, T), A = null;
      return D && (A = { type: "marginLeft", value: D }), e(T, C.rightside_text, M, A);
    }(r, d, u, _), x && v.appendChild(x), t._waiAria.setTaskBarAttr(r, b);
    var S = t.getState();
    return t.isReadonly(r) || (d.drag_resize && a(v, "gantt_task_drag", r, function(T) {
      var E = document.createElement("div");
      return E.className = T, E;
    }, d), d.drag_links && d.show_links && a(v, "gantt_link_control", r, function(T) {
      var E = document.createElement("div");
      E.className = T, E.style.cssText = ["height:" + h + "px", "line-height:" + h + "px"].join(";");
      var C = document.createElement("div");
      C.className = "gantt_link_point";
...
function fr(t) {
  var n = { current_pos: null, dirs: { left: "left", right: "right", up: "up", down: "down" }, path: [], clear: function() {
    ...
    var f = 2 * d.link_arrow_size, k = this.get_line_type(a, r.$getConfig()), m = l.e_x > l.x;
    if (k.from_start && k.to_start) this.point_to(h.left, f), m ? (this.point_to(h.down, u), this.point_to(h.right, c)) : (this.point_to(h.right, c), this.point_to(h.down, u)), this.point_to(h.right, f);
    else if (!k.from_start && k.to_start) if (u !== 0 && (m = l.e_x > l.x + 2 * f), this.point_to(h.right, f), m) c -= f, this.point_to(h.down, u), this.point_to(h.right, c);
    else {
      c -= 2 * f;
      var b = u > 0 ? 1 : -1;
      this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, c), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.right, f);
    }
    else k.from_start || k.to_start ? k.from_start && !k.to_start && (u !== 0 && (m = l.e_x > l.x - 2 * f), this.point_to(h.left, f), m ? (c += 2 * f, b = u > 0 ? 1 : -1, this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, c), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.left, f)) : (c += f, this.point_to(h.down, u), this.point_to(h.right, c))) : (this.point_to(h.right, f), m ? (this.point_to(h.right, c), this.point_to(h.down, u)) : (this.point_to(h.down, u), this.point_to(h.right, c)), this.point_to(h.left, f));
    return this.path;
  ...
  return { render: function(a, r, o) {
    ...
    const h = function(m, b) {
      const g = b.link_radius || 4, v = b.link_arrow_size || 6, p = [];
      for (let w = 0; w < m.length; w++) {
        const $ = m[w], x = m[w + 1];
        if (!x || b.link_radius <= 1) p.push({ type: "line", data: $ });
        else if ($.direction !== x.direction) {
          if ($.size < g || x.size < g) {
            p.push({ type: "line", data: $ });
            continue;
          }
          $.size -= g, p.push({ type: "line", data: $ });
          ...
          const E = { x: S, y: T, direction: { from: $.direction, to: x.direction }, radius: g };
      ...
      if (y.direction === "right" || y.direction === "left") {
        y.size -= 3 * v / 4;
        let w = y.direction === "right" ? y.x + y.size : y.x - y.size - v / 2, $ = y.y - b.link_line_width / 2 - v / 2 + 1;
        y.direction === "left" ? ($ -= 1, w -= 2) : w -= 1;
        const x = { x: w, y: $, size: v, direction: y.direction };
        p.push({ type: "line", data: y }), p.push({ type: "arrow", data: x });
      } else p.push({ type: "line", data: y });
      return p;
    }(n.get_lines(c, r).filter((m) => m.size > 0), o), _ = function(m, b, g, v) {
      const p = document.createElement("div");
      return m.forEach((y) => {
        let w;
        y.type === "line" ? w = n.render_line(y.data, null, b, g.source) : y.type === "corner" ? w = n.render_corner(y.data, b) : y.type === "arrow" && (w = n.render_arrow(y.data, v)), p.appendChild(w);
      }), p;
    }(h, r, a, o);
  ...
  }, update: null, isInViewPort: Jn, getVisibleRange: Yn() };
}
...
function Zn(t) {
  ...
  return t.$data.tasksStore.attachEvent("onStoreUpdated", function() {
    ...
    return s = n[o] ? n[o] : n[o] = function(l, d, u, c) {
      let h, _ = !1, f = {};
      t.config.process_resource_assignments && d === t.config.resource_property ? (h = l.$role == "task" ? t.getResourceAssignments(l.$resource_id, l.$task_id) : t.getResourceAssignments(l.id), _ = !0) : h = l.$role == "task" ? [] : t.getTaskBy(d, l.id), f = function(x, S, T) {
        ...
        for (let A = 0; A < x.length; A++) {
          ...
          let O = Bt(S.trace_x, P.valueOf()), B = new Date(S.trace_x[O] || t.date[E + "_start"](new Date(P))), H = new Date(Math.min(P.valueOf(), B.valueOf())), F = t.config.work_time ? t.getTaskCalendar(L) : t;
          for (M[F.id] = {}; H < N; ) {
            const V = M[F.id], j = H.valueOf();
            H = t.date.add(H, C, E), V[j] !== !1 && (D[j] || (D[j] = { tasks: [], assignments: [] }), D[j].tasks.push(L), T && D[j].assignments.push(I));
          }
        }
        return D;
      }(h, u, _);
      const k = u.unit, m = u.step, b = [];
      let g, v, p, y, w;
      const $ = c.$getConfig();
      for (let x = 0; x < u.trace_x.length; x++) g = new Date(u.trace_x[x]), v = t.date.add(g, m, k), w = f[g.valueOf()] || {}, p = w.tasks || [], y = w.assignments || [], p.length || $.resource_render_empty_cells ? b.push({ start_date: g, end_date: v, tasks: p, assignments: y }) : b.push(null);
      return b;
    }(e, i, a, r), s;
  };
}
...
function kr(t) {
  ...
  function o(l, d, u, c, h, _, f) {
    ...
    var m = function(b, g, v, p) {
      for (var y = g.getScale(), w = document.createElement("div"), $ = Ct(y, p), x = $.start; x <= $.end; x++) {
        var S = y.trace_x[x], T = y.trace_x[x + 1] || t.date.add(S, y.step, y.unit), E = y.trace_x[x].valueOf(), C = Math.min(b[E] / v, 1) || 0;
        if (C < 0) return null;
        var D = Math.min(b[T.valueOf()] / v, 1) || 0, M = mr(C, S, T, g);
        M && w.appendChild(M);
        var A = vr(C, D, g.posFromDate(T));
        A && w.appendChild(A);
      }
      return w;
    }(u, h, _, f);
  ...
  function s(l, d, u, c, h, _, f) {
    var k = h.histogram_cell_class(_.start_date, _.end_date, l, _.tasks, _.assignments), m = h.histogram_cell_label(_.start_date, _.end_date, l, _.tasks, _.assignments), b = h.histogram_cell_allocated(_.start_date, _.end_date, l, _.tasks, _.assignments), g = f.getItemHeight(l.id) - 1;
    if (k || m) {
      var v = document.createElement("div");
      return v.className = ["gantt_histogram_cell", k].join(" "), v.setAttribute(f.$config.item_attribute, l.id), v.style.cssText = ["left:" + d.left + "px", "width:" + d.width + "px", "height:" + g + "px", "line-height:" + g + "px", "top:" + (d.top + 1) + "px"].join(";"), m && (m = "<div class='gantt_histogram_label'>" + m + "</div>"), b && (m = "<div class='gantt_histogram_fill' style='height:" + 100 * Math.min(b / u || 0, 1) + "%;'></div>" + m), m && (v.innerHTML = m), v;
    }
    return null;
  }
  return { render: function(l, d, u, c) {
    var h = d.$getTemplates(), _ = d.getScale(), f = n(l, u.resource_property, _, d), k = [], m = {}, b = l.capacity || d.$config.capacity || 24;
    e[l.id] = {}, i[l.id] = null, a[l.id] = null;
    for (var g = !!c, v = Ct(_, c), p = v.start; p <= v.end; p++) {
      var y = f[p];
      if (y && (!g || Vt(p, _, c, t))) {
        var w = h.histogram_cell_capacity(y.start_date, y.end_date, l, y.tasks, y.assignments);
        m[y.start_date.valueOf()] = w || 0;
        var $ = d.getItemPosition(l, y.start_date, y.end_date), x = s(l, $, b, 0, h, y, d);
        x && (k.push(x), e[l.id][p] = x);
      }
    }
    var S = null;
    if (k.length) {
    ...
    var _ = u.$getTemplates(), f = u.getScale(), k = n(l, c.resource_property, f, u), m = l.capacity || u.$config.capacity || 24, b = {}, g = !!h, v = Ct(f, h), p = {};
    if (e && e[l.id]) for (var y in e[l.id]) p[y] = y;
    for (var w = v.start; w <= v.end; w++) {
      var $ = k[w];
      if (p[w] = !1, $) {
        var x = _.histogram_cell_capacity($.start_date, $.end_date, l, $.tasks, $.assignments);
        b[$.start_date.valueOf()] = x || 0;
        var S = u.getItemPosition(l, $.start_date, $.end_date);
        if (!g || Vt(w, f, h, t)) {
          var T = e[l.id];
    ...
    var C = o(l, S, b, 0, u, m, h);
    C && (d.appendChild(C), a[l.id] = C);
  }, getRectangle: He, getVisibleRange: Z };
}
...
function br(t) {
  ...
  function e(i, a, r) {
    ...
    let { height: d, marginTop: u } = Fn(t, a, l, 30, i, r), c = d, h = 0;
    switch (o) {
      case "snet":
      case "fnet":
      case "mso":
...
`;
  ...
  }, isInViewPort: yr, getVisibleRange: Z };
}
function $r(t, n, e, i, a) {
  return Je(t, n, e, 0, a, { start_date: "deadline", end_date: "deadline" });
}
function xr(t, n, e, i, a) {
  let r = !1;
  const o = { start_date: "start_date", end_date: "end_date" };
  return t.type == a.config.types.milestone && (o.end_date = o.start_date), t.baselines && (r = bn(t, n, e, i, a, o)), mt(t) && a.eachTask(function(s) {
...
const wr = { init: function(t, n) {
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(s, l) {
      ...
      var m = f.getItem(i.config.id), b = i._getTargetY(l), g = i._getTaskByY(b, f.getIndexById(m.id));
      function v(D, M) {
        return !f.isChildOf(p.id, M.id) && (D.$level == M.$level || _.order_branch_free);
      }
      if (f.exists(g) || (g = i.lastTaskOfLevel(_.order_branch_free ? m.$level : 0)) == i.config.id && (g = null), f.exists(g)) {
        var p = f.getItem(g), y = n.getItemTop(p.id), w = n.getItemHeight(p.id);
        if (y + w / 2 < b) {
          var $ = f.getIndexById(p.id), x = f.getNext(p.id), S = f.getItem(x);
          if (o(x)) {
            var T = f.getPrev(S.id);
            S = f.getItem(T);
          }
          if (S) {
            if (S.id == m.id) return _.order_branch_free && f.isChildOf(m.id, p.id) && f.getChildren(p.id).length == 1 ? void f.move(m.id, f.getBranchIndex(p.id) + 1, f.getParent(p.id)) : void 0;
            p = S;
          } else if (x = f.getIdByIndex($), S = f.getItem(x), o(x) && (T = f.getPrev(S.id), S = f.getItem(T)), v(S, m) && S.id != m.id) return void f.move(m.id, -1, f.getParent(S.id));
        } else if (_.order_branch_free && p.id != m.id && v(p, m) && !o(p.id)) {
          if (!f.hasChild(p.id)) return p.$open = !0, void f.move(m.id, -1, p.id);
        ...
        if (m.id == p.id || o(p.id)) return;
        v(p, m) && m.id != p.id ? f.move(m.id, 0, 0, p.id) : p.$level != m.$level - 1 || f.getChildren(p.id).length ? E && v(E, m) && m.id != E.id && f.move(m.id, -1, f.getParent(E.id)) : f.move(m.id, 0, p.id);
      }
      return !0;
  ...
  function a(s) {
    return Q(s, n.$config.item_attribute);
  }
  function r() {
...
} }, et = { createDropTargetObject: function(t) {
...
function Qn(t, n, e, i, a) {
  ...
  return i.exists(r) ? i.calculateItemLevel(i.getItem(r)) === e ? a ? et.nextSiblingTarget(t, r, i) : et.prevSiblingTarget(t, r, i) : et.firstChildTarget(t, r, i) : null;
}
function $e(t, n, e, i) {
  return Qn(t, n, e, i, !0);
}
...
function xn(t, n, e, i, a, r) {
  ...
  if (n !== a.$getRootId()) {
    ...
    if (l === r) {
      ...
      e < 0.5 && !d ? o = et.prevSiblingTarget(t, n, a) : (e < 0.5 && (n = d), o = et.nextSiblingTarget(t, n, a));
    } else if (l > r) a.eachParent(function(f) {
      a.calculateItemLevel(f) === r && (n = f.id);
    }, s), o = $e(t, n, r, a);
    else {
      var u = $e(t, n, r, a), c = $n(t, n, r, a);
      o = e < 0.5 ? u : c;
    }
  } else {
    var h = a.$getRootId(), _ = a.getChildren(h);
    o = et.createDropTargetObject(), o = _.length && i >= 0 ? $e(t, function(f) {
      for (var k = f.getNext(); f.exists(k); ) {
        var m = f.getNext(k);
        if (!f.exists(m)) return k;
        k = m;
      }
      return null;
    }(a), r, a) : $n(t, h, r, a);
...
const Sn = { removeLineHighlight: function(t) {
  ...
  var i = function(r, o) {
    ...
    return u > s.y + h - c && (u = s.y + h - c), s.x = d, s.y = u, s.width = xe(o.$gantt, s.width, 9), s;
  }(t, e);
  ...
  a || ((a = document.createElement("div")).className = "gantt_drag_marker gantt_grid_dnd_marker", a.innerHTML = "<div class='gantt_grid_dnd_marker_line'></div>", a.style.pointerEvents = "none"), t.child ? function(r, o, s) {
    ...
    let c = xe(s.$gantt, s.$grid_data.offsetWidth);
    ...
    var l = function(c, h) {
      var _ = h.$config.rowStore, f = { x: 0, y: 0 }, k = h.$grid_data.querySelector(".gantt_tree_indent"), m = 15, b = 0;
      k && (m = k.offsetWidth);
      var g = 40;
      if (c.targetId !== _.$getRootId()) {
        var v = h.getItemTop(c.targetId), p = h.getItemHeight(c.targetId);
        if (b = _.exists(c.targetId) ? _.calculateItemLevel(_.getItem(c.targetId)) : 0, c.prevSibling) f.y = v;
        else if (c.nextSibling) {
          var y = 0;
          _.eachItem(function(w) {
            _.getIndexById(w.id) !== -1 && y++;
          }, c.targetId), f.y = v + p + y * p;
        } else f.y = v + p, b += 1;
      }
      return f.x = g + b * m, f.width = xe(h.$gantt, Math.max(h.$grid_data.offsetWidth - f.x, 0), f.x), wn(f, h);
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(s, l) {
      ...
      i.config.level = c.calculateItemLevel(h), i.config.drop_target = et.createDropTargetObject({ targetParent: c.getParent(h.id), targetIndex: c.getBranchIndex(h.id), targetId: h.id, nextSibling: !0 }), h.$open = !1, h.$transparent = !0, this.refreshData();
    }, t)), i.attachEvent("onDragMove", t.bind(function(s, l) {
      var d = o(l);
      return d && t.callEvent("onBeforeRowDragMove", [i.config.id, d.targetParent, d.targetIndex]) !== !1 || (d = et.createDropTargetObject(i.config.drop_target)), Sn.highlightPosition(d, i.config, n), i.config.drop_target = d, t._waiAria.reorderMarkerAttr(i.config.marker), this.callEvent("onRowDragMove", [i.config.id, d.targetParent, d.targetIndex]), !0;
    }, t)), i.attachEvent("onDragEnd", t.bind(function() {
  ...
  function o(s) {
    var l, d = function(f) {
      var k = ct(f, n.$grid_data).y, m = n.$config.rowStore;
      document.doctype || (k += window.scrollY), k = k || 0;
      var b = n.$state.scrollTop || 0, g = t.$grid_data.getBoundingClientRect().height + b + window.scrollY, v = b, p = n.getItemIndexByTopPosition(n.$state.scrollTop);
      if (m.exists(p) || (p = m.countVisible() - 1), p < 0) return m.$getRootId();
      var y = m.getIdByIndex(p), w = n.$state.scrollTop / n.getItemHeight(y), $ = w - Math.floor(w);
      $ > 0.1 && $ < 0.9 && (g -= n.getItemHeight(y) * $, v += n.getItemHeight(y) * (1 - $));
      const x = J(n.$grid_data), S = x.y + x.height, T = i.config.marker.offsetHeight;
      k + T + window.scrollY >= g && (i.config.marker.style.top = S - T + "px"), k >= g ? k = g : k <= v && (k = v, i.config.marker.style.top = x.y + "px");
      var E = n.getItemIndexByTopPosition(k);
      if (E > m.countVisible() - 1 || E < 0) return m.$getRootId();
      var C = m.getIdByIndex(E);
      return r(C) ? m.getPrevSibling(C) : m.getIdByIndex(E);
    }(s), u = null, c = n.$config.rowStore, h = !n.$getConfig().order_branch_free, _ = ct(s, n.$grid_data).y;
    return document.doctype || (_ += window.scrollY), d !== c.$getRootId() && (u = (_ - n.getItemTop(d)) / n.getItemHeight(d)), h ? (l = xn(i.config.id, d, u, _, c, i.config.level)) && l.targetParent && r(l.targetParent) && (d = c.getPrevSibling(l.targetParent), l = xn(i.config.id, d, u, _, c, i.config.level)) : l = function(f, k, m, b, g) {
      var v;
      if (k !== g.$getRootId()) v = m < 0.25 ? et.prevSiblingTarget(f, k, g) : !(m > 0.6) || g.hasChild(k) && g.getItem(k).$open ? et.firstChildTarget(f, k, g) : et.nextSiblingTarget(f, k, g);
      else {
        var p = g.$getRootId();
        v = g.hasChild(p) && b >= 0 ? et.lastChildTarget(f, p, g) : et.firstChildTarget(f, p, g);
      }
      return v;
...
var Tr = function(t) {
  return { onCreated: function(n) {
    n.$config = R(n.$config, { bind: "task" }), n.$config.id == "grid" && (this.extendGantt(n), t.ext.inlineEditors = t.ext._inlineEditors.createEditors(n), t.ext.inlineEditors.init()), this._mouseDelegates = Ge(t);
  }, onInitialized: function(n) {
    var e = n.$getConfig();
    e.order_branch && (e.order_branch == "marker" ? Sr.init(n.$gantt, n) : wr.init(n.$gantt, n)), this.initEvents(n, t), n.$config.id == "grid" && this.extendDom(n);
    ...
    this._mouseDelegates.delegate("click", "gantt_row", e.bind(function(i, a, r) {
      ...
      if (a !== null) {
        ...
        if (o.scroll_on_click) {
          ...
          l && d && this.showDate(s.start_date);
        }
...
const Er = { createTaskDND: function() {
  var t;
      ...
      return { drag: null, dragMultiple: {}, _events: { before_start: {}, before_finish: {}, after_finish: {} }, _handlers: {}, init: function() {
        ...
        if (s.mode) {
          ...
          if (this._handlers[s.mode]) {
            if (s.mode === l.drag_mode.move) {
              ...
              var m = this._find_max_shift(_, c);
              let v;
              if (m !== void 0 && (c = m), this._update_item_on_move(c, s.id, s.mode, s, o), m === void 0) {
                const p = a.posFromDate(s.obj.start_date), y = a.posFromDate(s.obj.end_date);
                if (s.handle_offset === void 0) {
                  const $ = y - p, x = s.start_x - p;
                  s.handle_offset = x / $;
                }
              ...
              for (var b in _) {
                var g = _[b];
                f && g.id != s.id && (a._bulk_dnd = !0), this._update_item_on_move(c, g.id, g.mode, g, o, v);
              }
              a._bulk_dnd = !1;
        ...
        if (o.button != 2 || o.button === void 0) {
          ...
          if (a.isTaskExists(d) && (u = a.getTask(d)), !a.isReadonly(u) && !this.drag.mode) {
            ...
            var c = nt(s = s || o.target || o.srcElement), h = this._get_drag_mode(c, s);
...
var Cr = function(t, n) {
  ...
  function f(v, p) {
    var y, w = _.getPosition(v), $ = function(M) {
      var A = 0, I = 0;
      return M && (A = M.offsetWidth || 0, I = M.offsetHeight || 0), { width: A, height: I };
    }(p), x = { right: (y = n.$root).offsetWidth, bottom: y.offsetHeight }, S = n.config.tooltip_offset_x || 10, T = n.config.tooltip_offset_y || 10, E = n.config.scroll_size || 18, C = n.$container.getBoundingClientRect().y + window.scrollY, D = { y: w.y + T, x: w.x + S, bottom: w.y + $.height + T + E, right: w.x + $.width + S + E };
    return D.bottom > x.bottom + C && (D.y = x.bottom + C - $.height - T), D.right > x.right && (D.x = x.right - $.width - S), D;
  }
  function k(v) {
    var p = s();
  ...
  function b(v, p, y, w) {
    ...
    }(), x = s(), S = ["gantt_link_direction"];
    n.templates.link_direction_class && S.push(n.templates.link_direction_class(x.link_source_id, x.link_from_start, x.link_target_id, x.link_to_start));
    var T = Math.sqrt(Math.pow(y - v, 2) + Math.pow(w - p, 2));
    if (T = Math.max(0, T - 3)) {
      $.className = S.join(" ");
      var E = (w - p) / (y - v), C = Math.atan(E);
      g(v, y, p, w) == 2 ? C += Math.PI : g(v, y, p, w) == 3 && (C -= Math.PI);
      var D = Math.sin(C), M = Math.cos(C), A = Math.round(p), I = Math.round(v), L = ["-webkit-transform: rotate(" + C + "rad)", "-moz-transform: rotate(" + C + "rad)", "-ms-transform: rotate(" + C + "rad)", "-o-transform: rotate(" + C + "rad)", "transform: rotate(" + C + "rad)", "width:" + Math.round(T) + "px"];
      if (window.navigator.userAgent.indexOf("MSIE 8.0") != -1) {
        L.push('-ms-filter: "' + function(O, B) {
          return "progid:DXImageTransform.Microsoft.Matrix(M11 = " + B + ",M12 = -" + O + ",M21 = " + O + ",M22 = " + B + ",SizingMethod = 'auto expand')";
        }(D, M) + '"');
        var P = Math.abs(Math.round(v - y)), N = Math.abs(Math.round(w - p));
        switch (g(v, y, p, w)) {
          case 1:
            A -= N;
            break;
      ...
      L.push("top:" + A + "px"), L.push("left:" + I + "px"), $.style.cssText = L.join(";");
    }
  }
  function g(v, p, y, w) {
    return p >= v ? w <= y ? 1 : 4 : w <= y ? 2 : 3;
  }
  _.attachEvent("onBeforeDragStart", n.bind(function(v, p) {
    var y = p.target || p.srcElement;
    if (m(), n.getState("tasksDnd").drag_id) return !1;
    if (yt(y, c)) {
      yt(y, "task_start_date") && (o = !0);
      var w = n.locate(p);
      r = w;
      var $ = n.getTask(w);
      return n.isReadonly($) ? (m(), !1) : (this._dir_start = { x: _.config.original_element_sizes.x + _.config.original_element_sizes.width / 2, y: _.config.original_element_sizes.y + _.config.original_element_sizes.height / 2 }, !0);
    }
    return !1;
  }, this)), _.attachEvent("onAfterDragStart", n.bind(function(v, p) {
    n.config.touch && n.refreshData(), k(_.config.marker);
  }, this)), _.attachEvent("onDragMove", n.bind(function(v, p) {
    var y = _.config, w = f(p, y.marker);
    (function(M, A) {
      M.style.left = A.x + "px", M.style.top = A.y + "px";
    })(y.marker, w);
    var $ = !!yt(p, h), x = i, S = e, T = a, E = n.locate(p), C = !0;
    if (tt(At(p), n.$root) || ($ = !1, E = null), $ && (C = !yt(p, "task_end_date"), $ = !!E), i = E, e = $, a = C, $) {
      const M = yt(p, h).querySelector(`.${c}`);
      if (M) {
        const A = In(M, t.$task_bg);
    ...
    var D = !(S == $ && x == E && T == C);
    return D && (x && n.refreshTask(x, !1), E && n.refreshTask(E, !1)), D && k(y.marker), b(this._dir_start.x, this._dir_start.y, this._dir_end.x, this._dir_end.y), !0;
  }, this)), _.attachEvent("onDragEnd", n.bind(function() {
    var v = s();
    if (v.link_source_id && v.link_target_id && v.link_source_id != v.link_target_id) {
      var p = n._get_link_type(v.link_from_start, v.link_to_start), y = { source: v.link_source_id, target: v.link_target_id, type: p };
      y.type && n.isLinkAllowed(y) && n.callEvent("onLinkCreated", [y]) && n.addLink(y);
    }
    m(), n.config.touch ? n.refreshData() : (v.link_source_id && n.refreshTask(v.link_source_id, !1), v.link_target_id && n.refreshTask(v.link_target_id, !1)), _._direction && (_._direction.parentNode && _._direction.parentNode.removeChild(_._direction), _._direction = null);
...
var Ar = function(t) {
  ...
  return { onCreated: function(e) {
    var i = e.$config;
    i.bind = G(i.bind) ? i.bind : "task", i.bindLinks = G(i.bindLinks) ? i.bindLinks : "link", e._linksDnD = Dr(), e._tasksDnD = Er.createTaskDND(), e._tasksDnD.extend(e), this._mouseDelegates = Ge(t);
    ...
    this._mouseDelegates.delegate("click", "gantt_task_link", e.bind(function(a, r) {
      ...
      return o.parentNode && nt(o.parentNode) && (l = nt(o.parentNode).indexOf("_left") > -1 ? s.$target[0] : s.$source[0]), l && i.call(this, l, a), !1;
    }, e), this.$task);
    ...
    this._legacyGridResizerClass(n), n.attachEvent("onBeforeResize", function() {
      ...
      if (a) {
        ...
        if (e && !o && r !== !1 && (t.config.grid_width = r), a.$config.hidden = a.$parent.$config.hidden = o, !a.$config.hidden) {
          ...
          } else i && tt(i.$task, n.$view) && (a.$config.original_grid_width = t.config.grid_width), e || (a.$parent.$config.width = 0);
        }
        e = !1;
      }
...
const Mr = { init: function(t) {
  ...
  var e = ja(t);
  e.registerView("cell", wt), e.registerView("resizer", Ya), e.registerView("scrollbar", Ja), e.registerView("layout", Kn, function(r) {
    (r.$config ? r.$config.id : null) === "main" && n(r, Ir);
  }), e.registerView("viewcell", qa), e.registerView("multiview", Ga), e.registerView("timeline", qe, function(r) {
    (r.$config ? r.$config.id : null) !== "timeline" && r.$config.bind != "task" || n(r, Ar);
  }), e.registerView("grid", ue, function(r) {
    (r.$config ? r.$config.id : null) !== "grid" && r.$config.bind != "task" || n(r, Tr);
  }), e.registerView("resourceGrid", Za), e.registerView("resourceTimeline", Xn), e.registerView("resourceHistogram", Qa);
  var i = function(r) {
    var o = Va(r);
    return { getDataRender: function(s) {
      ...
      return { addTaskLayer: function(d) {
        ...
        const u = Yn();
        return typeof d == "function" ? d = { renderer: { render: d, getVisibleRange: u } } : d.renderer && !d.renderer.getVisibleRange && (d.renderer.getVisibleRange = u), d.view = "timeline", d && d.renderer && (d.renderer.getRectangle || d.renderer.isInViewPort || (d.renderer.isInViewPort = Jn)), l.addLayer(d);
  ...
  return t.ext.inlineEditors = a, t.ext._inlineEditors = a, a.init(t), { factory: e, mouseEvents: Wa.init(t), layersApi: i.init(), render: { gridLine: function() {
    return /* @__PURE__ */ function(r) {
      return { render: function(o, s, l, d) {
        for (var u = s.getGridColumns(), c = s.$getTemplates(), h = s.$config.rowStore, _ = [], f = 0; f < u.length; f++) {
          var k, m, b, g = f == u.length - 1, v = u[f];
          v.name == "add" ? (m = "<div " + (S = r._waiAria.gridAddButtonAttrString(v)) + " class='gantt_add'></div>", b = "") : (it(m = v.template ? v.template(o) : o[v.name]) && (m = c.date_grid(m, o, v.name)), m == null && (m = ""), b = m, m = "<div class='gantt_tree_content'>" + m + "</div>");
          ...
          var $ = "width:" + (v.width - (g ? 1 : 0)) + "px;";
          if (this.defined(v.align)) {
            var x = { right: "flex-end", left: "flex-start", center: "center" }[v.align];
            $ += "text-align:" + v.align + ";justify-content:" + x + ";";
          }
          var S = r._waiAria.gridCellAttrString(v, b, o);
          y.push(m), k = "<div class='" + p + "' data-column-index='" + f + "' data-column-name='" + v.name + "' style='" + $ + "' " + S + ">" + y.join("") + "</div>", _.push(k);
        }
      ...
      }, update: null, getRectangle: ie, isInViewPort: pr, getVisibleRange: Z, onrender: function(o, s, l) {
    ...
    return /* @__PURE__ */ function(r) {
      ...
      function h(_, f, k, m, b, g, v, p) {
        var y = _.width[f], w = "";
        if (Vt(f, _, m, r)) {
          var $ = g(k, _.trace_x[f]), x = "";
          if (v && (x = v(k, _.trace_x[f])), p.static_background) {
            var S = !(!$ && !x);
            if (!p.static_background_cells || !S) return null;
          }
          if (o[k.id][f]) return s[k.id][f] = f, o[k.id][f];
          var T = document.createElement("div");
          return T.style.width = y + "px", w = "gantt_task_cell" + (f == b - 1 ? " gantt_last_cell" : ""), $ && (w += " " + $), T.className = w, x && (T.innerHTML = x), T.style.position = "absolute", T.style.left = _.left[f] + "px", o[k.id][f] = T, s[k.id][f] = f, T;
        }
        return null;
      }
      return { render: function(_, f, k, m) {
        var b = f.$getTemplates(), g = f.getScale(), v = g.count;
        if (k.static_background && !k.static_background_cells) return null;
        var p, y = document.createElement("div"), w = u(f), $ = c(f);
        if (p = m && k.smart_rendering && !Wt(r) ? Ct(g, m.x) : { start: 0, end: v - 1 }, k.show_task_cells) {
          o[_.id] = {}, s[_.id] = {};
          for (var x = p.start; x <= p.end; x++) {
            var S = h(g, x, _, m, v, w, $, k);
            S && y.appendChild(S);
          }
        }
        ...
        if (T.isSelected(_.id) && (D += " gantt_selected"), y.className = D, k.smart_rendering ? (y.style.position = "absolute", y.style.top = f.getItemTop(_.id) + "px", y.style.width = "100%") : y.style.position = "relative", y.style.height = f.getItemHeight(_.id) + "px", _.id == "timeline_placeholder_task") {
          ...
          var A = (_.row_height || f.$task_data.offsetHeight) - M;
          A < 0 && (A = 0), k.smart_rendering && (y.style.top = M + "px"), y.style.height = A + "px";
        }
        return f.$config.item_attribute && (y.setAttribute(f.$config.item_attribute, _.id), y.setAttribute(f.$config.bind + "_id", _.id)), y;
      }, update: function(_, f, k, m, b) {
        var g = k.getScale(), v = g.count, p = u(k), y = c(k);
        if (m.show_task_cells) {
          o[_.id] || (o[_.id] = {}), s[_.id] || (s[_.id] = {});
          var w = Ct(g, b);
          for (var $ in s[_.id]) {
            var x = s[_.id][$];
            (Number(x) < w.start || Number(x) > w.end) && d(_.id, x);
          }
          s[_.id] = {};
          for (var S = w.start; S <= w.end; S++) {
            var T = h(g, S, _, b, v, p, y, m);
            !T && l(_, S) ? d(_.id, S) : T && !T.parentNode && f.appendChild(T);
          }
        }
      }, getRectangle: He, getVisibleRange: Z, prepareData: gr };
    }(t);
  }, taskBar: function() {
    return function(r) {
      return { render: be(r), update: null, isInViewPort: Mt, getVisibleRange: Z };
    }(t);
  }, timedProjectBar: function() {
    return function(r) {
      return { render: hr(r), update: null, isInViewPort: ur, getVisibleRange: Z };
    }(t);
    ...
    return function(r) {
      ...
      function l(c, h, _, f, k) {
        let m = !0;
        return f.smart_rendering && (m = Mt(c, h, _)), m;
      }
      function d(c, h, _, f) {
        ...
        return m.style.top = f.top + g + "px", m.classList.add("gantt_rollup_child"), m.setAttribute("data-rollup-parent-id", c.id), m;
      }
      function u(c, h) {
        return c + "_" + h;
      }
      return { render: function(c, h, _, f) {
        if (c.rollup !== !1 && c.$rollup && c.$rollup.length) {
          const k = document.createElement("div"), m = r.getTaskPosition(c);
          return f && (f.y = 0, f.y_end = r.$task_bg.scrollHeight), c.$rollup.forEach(function(b) {
            if (!r.isTaskExists(b)) return;
            const g = r.getTask(b);
            if (!l(g, f, h, _)) return;
            const v = d(c, g, h, m);
            v ? (s[u(g.id, c.id)] = v, k.appendChild(v)) : s[u(g.id, c.id)] = !1;
          }), k;
        }
        return !1;
      }, update: function(c, h, _, f, k) {
        const m = document.createElement("div"), b = r.getTaskPosition(c);
        k.y = 0, k.y_end = r.$task_bg.scrollHeight, c.$rollup.forEach(function(g) {
          const v = r.getTask(g), p = u(v.id, c.id);
          let y = l(v, k, _, f);
          if (y !== !!s[p]) if (y) {
            const w = d(c, v, _, b);
            s[p] = w || !1;
          } else s[p] = !1;
          s[p] && m.appendChild(s[p]), h.innerHTML = "", h.appendChild(m);
        });
    ...
    return function(r) {
      const o = be(r), s = {};
      function l(h, _, f, k, m) {
        let b = !h.hide_bar;
        return k.smart_rendering && b && (b = Mt(h, _, f)), b;
      }
      function d(h, _, f, k, m) {
        ...
        let y;
        const w = k.rowHeight, $ = f.getBarHeight(g.id, p);
        let x = Math.floor((f.getItemHeight(h.id) - $) / 2);
        p && (y = hn($)), m && (x = p ? Math.floor((y - $) / 2) + 2 : 2);
        const S = v.querySelector(".gantt_link_control.task_start_date"), T = v.querySelector(".gantt_link_control.task_end_date");
        if (p) {
          if (y > w) {
            x = 2, v.style.height = w - x + "px";
            const E = _n(w), C = (E - w) / 2, D = v.querySelector(".gantt_task_content");
            x = Math.abs(C), S.style.marginLeft = C + "px", T.style.marginRight = C + "px", S.style.height = T.style.height = E + "px", v.style.width = D.style.height = D.style.width = E + "px", v.style.left = f.getItemPosition(g).left - E / 2 + "px";
          }
        } else $ + x > w && (x = 0, v.style.height = v.style.lineHeight = S.style.height = T.style.height = w + "px");
        return v.style.top = k.top + x + "px", v.classList.add("gantt_split_child"), b && v.classList.add("gantt_split_subproject"), v;
      }
      function u(h, _) {
        return h + "_" + _;
      }
      ...
      return { render: function(h, _, f, k) {
        if (c(h, f)) {
          const m = document.createElement("div"), b = r.getTaskPosition(h), g = Ft(r, h.id);
          return r.hasChild(h.id) && r.eachTask(function(v) {
            if (!l(v, k, _, f)) return;
            const p = d(h, v, _, b, g);
            p ? (s[u(v.id, h.id)] = p, m.appendChild(p)) : s[u(v.id, h.id)] = !1;
          }, h.id), m;
        }
        return !1;
      }, update: function(h, _, f, k, m) {
        if (c(h, k)) {
          const b = document.createElement("div"), g = r.getTaskPosition(h), v = Ft(r, h.id);
      ...
      }, isInViewPort: _r, getVisibleRange: Z };
    }(t);
    ...
    return /* @__PURE__ */ function(r) {
      function o(s, l, d) {
        const u = document.createElement("div"), c = r.getTaskPosition(s, s.deadline, s.deadline), { height: h, marginTop: _ } = Fn(r, l, c, 20, s, d);
        let f = h;
    ...
    return /* @__PURE__ */ function(r) {
      function o(s, l, d, u, c) {
        ...
        if (f) {
          ...
          m = hn(T), b = Math.floor((m - T) / 4);
        }
        let g = ne(r, u, s, k.rowHeight).maxHeight, v = k.top + 1 + b, p = u.getBarHeight(s.id, s.type);
        const y = r.config.baselines.row_height, w = r.config.baselines.bar_height;
        let $, x;
        switch (r.config.baselines.render_mode) {
          case "separateRow":
            v += k.height + (y - w) / 2, p = w;
            break;
          case "individualRow":
            $ = y * d, v += k.height + $ + (y - w) / 2, p = w;
            break;
          default:
            x = 1, c ? (g = ne(r, u, s).maxHeight, b ? g >= m ? x = (g - p) / 2 - 1 - b : (p = _n(g), v = k.top, x = Math.abs(p - g) / 2, b = 0) : (g > p && (x = (g - p) / 2 - 1), x -= b), s.bar_height || (x -= 1)) : (s.bar_height && k.rowHeight >= s.bar_height && (x = (k.rowHeight - s.bar_height) / 2 - 1), x += b, s.bar_height || (x += 2), f && (x += 1));
        }
        let S = k.top + g + 1 - b;
        return !(v + p > S && (p -= v + p - S, p <= 0)) && (h.style.left = k.left + "px", h.style.width = k.width + "px", h.style.top = v + "px", h.style.height = Math.floor(p) + "px", x && (h.style.marginTop = x + "px"), h.className = `gantt_task_baseline gantt_task_baseline_${d} ${l.className || ""}`, f ? (h.className += "gantt_milestone_baseline", h.style.width = h.style.height = p + "px", h.style.marginLeft = Math.floor(-p / 2) + "px") : h.innerHTML = r.templates.baseline_text(s, l, d), h.setAttribute("data-task-id", s.id), h.setAttribute("data-baseline-id", l.id), h);
      }
      return { render: function(s, l, d, u) {
    ...
    return function(r) {
      ...
      function l(u, c, h, _, f) {
        var k = h.resource_cell_class(c.start_date, c.end_date, u, c.tasks, c.assignments), m = h.resource_cell_value(c.start_date, c.end_date, u, c.tasks, c.assignments), b = f.getItemHeight(u.id) - 1;
        if (k || m) {
          var g = f.getItemPosition(u, c.start_date, c.end_date), v = document.createElement("div");
          return v.setAttribute(f.$config.item_attribute, u.id), v.className = ["gantt_resource_marker", k].join(" "), v.style.cssText = ["left:" + g.left + "px", "width:" + g.width + "px", "height:" + b + "px", "line-height:" + b + "px", "top:" + g.top + "px"].join(";"), m && (v.innerHTML = m), v;
        }
        return null;
      }
      function d(u, c) {
      ...
      return { render: function(u, c, h, _) {
        var f = c.$getTemplates(), k = c.getScale(), m = o(u, h.resource_property, c.getScale(), c), b = !!_, g = [];
        s[u.id] = {};
        for (var v = Ct(k, _), p = v.start; p <= v.end; p++) {
          var y = m[p];
          if (y && (!b || Vt(p, k, _, r))) {
            var w = l(u, y, f, 0, c);
        ...
        if (g.length) {
          ...
          for (var x = 0; x < g.length; x++) $.appendChild(g[x]);
        }
        return $;
      }, update: function(u, c, h, _, f) {
        var k = h.$getTemplates(), m = h.getScale(), b = o(u, _.resource_property, h.getScale(), h), g = Ct(m, f), v = {};
        if (s && s[u.id]) for (var p in s[u.id]) v[p] = p;
        for (var y = g.start; y <= g.end; y++) {
          var w = b[y];
          if (v[y] = !1, w) if (Vt(y, m, f, r)) if (s[u.id] && s[u.id][y]) s[u.id] && s[u.id][y] && !s[u.id][y].parentNode && c.appendChild(s[u.id][y]);
          else {
            var $ = l(u, w, k, 0, h);
            $ && (c.appendChild($), s[u.id][y] = $);
          }
          else d(u.id, y);
        }
        for (var p in v) v[p] !== !1 && d(u.id, p);
      }, getRectangle: He, getVisibleRange: Z };
    }(t);
    ...
    return /* @__PURE__ */ function(r) {
      ...
      }, update: null, getRectangle: ie, getVisibleRange: Z };
    }(t);
...
function Lr(t) {
  ...
  function u(b) {
    return b && tt(b, t.$root) && b.offsetHeight;
  }
  function c() {
    var b = !!document.querySelector(".gantt_drag_marker"), g = !!document.querySelector(".gantt_drag_marker.gantt_grid_resize_area") || !!document.querySelector(".gantt_drag_marker.gantt_row_grid_resize_area"), v = !!document.querySelector(".gantt_link_direction"), p = t.getState(), y = p.autoscroll;
    return o = b && !g && !v, !(!p.drag_mode && !b || g) || y;
  }
  function h(b) {
  ...
  function k() {
    ...
    if (b) {
      ...
      var v = J(b), p = d.x - v.x, y = d.y - v.y + window.scrollY, w = o ? 0 : m(p, v.width, l.x - v.x), $ = m(y, v.height, l.y - v.y + window.scrollY), x = t.getScrollState(), S = x.y, T = x.inner_height, E = x.height, C = x.x, D = x.inner_width, M = x.width;
      ($ && !T || $ < 0 && !S || $ > 0 && S + T >= E + 2) && ($ = 0), (w && !D || w < 0 && !C || w > 0 && C + D >= M) && (w = 0);
      var A = t.config.autoscroll_step;
      A && A < 2 && (A = 2), $ *= A || e, ((w *= A || e) || $) && function(I, L) {
        var P = t.getScrollState(), N = null, O = null;
        I && (N = P.x + I, N = Math.min(P.width, N), N = Math.max(0, N)), L && (O = P.y + L, O = Math.min(P.height, O), O = Math.max(0, O)), t.scrollTo(N, O);
      }(w, $);
    }
  }
  function m(b, g, v) {
    return b - n < 0 && b < v ? -1 : b > g - n && b > v ? 1 : 0;
  }
  t.attachEvent("onGanttReady", function() {
    if (!Y(t)) {
...
const Pr = null, Rr = ["ctrlKey", "altKey", "shiftKey", "metaKey"], Hr = [[{ unit: "month", date: "%M", step: 1 }, { unit: "day", date: "%d", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 8 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 1 }]];
class Or {
  constructor(n) {
    this.zoomIn = () => {
...
function Br(t) {
  ...
  t._touch_events = function(r, o, s) {
    var l, d = 0, u = !1, c = !1, h = null, _ = null, f = null, k = [], m = null;
    let b = {};
    ...
    (a = []).push([t.$container, r[0], function(p) {
      ...
      if (!s(p) && u) {
        ...
        if (!t._prevent_touch_scroll) {
          if (w && h) {
            var $ = h.pageX - w.pageX, x = h.pageY - w.pageY;
            if (!c && (Math.abs($) > 5 || Math.abs(x) > 5) && (c = !0, d = 0, l = m ? e(m) : t.getScrollState()), c) {
              var S, T = l.x + $, E = l.y + x;
              if (m ? (function(C, D, M) {
              ...
              }(m, T, E), S = e(m)) : (t.scrollTo(T, E), S = t.getScrollState()), l.x != S.x && x > 2 * $ || l.y != S.y && $ > 2 * x) return v(p);
            }
          }
          return v(p);
        }
    ...
    for (a.push([this.$container, "contextmenu", function(p) {
      ...
      if (b = p.touches.length, document && document.body && document.body.classList.add("gantt_touch_active"), !s(p)) if (p.touches && p.touches.length > 1) u = !1;
        ...
        h = o(p), m = function(w) {
          for (var $ = t.$layout.getCellsByType("viewCell"), x = 0; x < $.length; x++) {
        ...
        _ = setTimeout(function() {
          var w = t.locate(h);
          y && w && !t._locate_css(h, "gantt_link_control") && !t._locate_css(h, "gantt_grid_data") && (y.on_mouse_down(h), y.drag && y.drag.start_drag && (function($) {
            ...
            if (S) {
              ...
              if (T) {
                f = $;
                for (let E = 0; E < x.length; E++) if (S = x[E].rendered[$], S && S.getAttribute(t.config.task_attribute) && S.getAttribute(t.config.task_attribute) == $) {
                  const C = S.cloneNode(!0);
                  k.push(S), x[E].rendered[$] = C, S.style.display = "none", C.className += " gantt_drag_move ", S.parentNode.appendChild(C);
                }
      ...
      if (document && document.body && document.body.classList.remove("gantt_touch_active"), !s(p)) {
        ...
        var y = o(p), w = i();
        if (w && w.on_mouse_up(y), f && t.isTaskExists(f) && (t.refreshTask(f), k.length && (k.forEach(function(x) {
          x.parentNode && x.parentNode.removeChild(x);
        }), t._touch_feedback())), u = c = !1, k = [], f = null, h && d) {
          var $ = /* @__PURE__ */ new Date();
          $ - d < 500 && b <= 1 ? (t.$services.getService("mouseEvents").onDoubleClick(h), v(p)) : d = $;
...
function Ot(t, n, e) {
  return n = n || [], "<" + t + ti(e || []) + ">" + (n.innerHTML || "") + "</" + t + ">";
}
function ti(t) {
  var n = "";
  ...
  function i(a, r) {
    ...
    r && (o = t.getTaskByTime(), a.allow_root && o.unshift({ id: t.config.root_id, text: a.root_label || "" }), o = function(c, h, _) {
      ...
      for (var k = 0; k < c.length; k++) {
        var m = c[k];
        (m.id == _ || t.isChildOf(m.id, _) || f(m.id, m) === !1) && (c.splice(k, 1), k--);
      }
      return c;
...
function jr(t) {
  ...
  return t.attachEvent("onAfterLightbox", function() {
    ...
    var h, _ = function(k, m) {
      return e.resources[m.id] || (e.resources[m.id] = k.querySelector(".gantt_resources")), e.resources[m.id];
    }(l, c), f = "";
    (function(k, m) {
      if (!e.filter[m.id]) {
        var b = k.querySelector(".gantt_resources_filter"), g = b.querySelector(".gantt_resources_filter_input"), v = b.querySelector(".switch_unsetted");
        e.filter[m.id] = { container: b, input: g, checkbox: v, filterApplied: !1 };
      }
      e.filter[m.id];
    })(l, c), function(k, m, b, g) {
      if (e.eventsInitialized[b.id]) return;
      var v = function(S) {
        var T, E, C, D, M;
        y(b, k);
        var A = s(b);
        M = A.checkbox, D = A.input, C = M.checked, E = D.value.trim(), A.filterApplied = !!E, t.getState().lightbox && (m = t.getLightboxValues()), T = function(L, P, N, O) {
          var B, H;
          if (O) {
            var F = P[L.map_to] || [];
            ...
            for (var rt = {}, vt = 0; vt < F.length; vt++) rt[F[vt].resource_id] = !0;
            B = function(q) {
              if (rt[String(q.key)] && (N === "" || q.label.toLowerCase().indexOf(N.toLowerCase()) >= 0)) return q;
            };
          } else {
            if (N === "") return L;
            B = function(q) {
        ...
        var I = m[b.map_to];
        g.form_blocks.resources.set_value(k, I, m, T);
      };
      function p(S) {
        var T, E = S.target;
        if (S.target.type === "checkbox") {
          ...
          if (D.setAttribute("data-checked", E.checked), E.checked) {
            ...
            var A = C, I = b.default_value;
            b.options.forEach(function(L) {
              L.key == A && L.default_value && (I = L.default_value);
            }), M && !M.value && I !== void 0 && (M.value = I, y(b, this)), M.select ? M.select() : M.focus && M.focus();
      ...
      function y(S, T) {
        ...
        for (var D = 0; D < C.length; D++) {
          var M = C[D].getAttribute("data-item-id"), A = C[D].getAttribute("data-assignment-id");
          C[D].disabled ? delete e.resourcesValues[S.id][M] : e.resourcesValues[S.id][M] = { value: C[D].value, id: A };
        }
      }
      w = v, $ = 100, x = !1, v = function() {
        x || (w.apply(null, arguments), x = !0, setTimeout(function() {
          x = !1;
        }, $));
      }, s(b).container.addEventListener("keyup", v), s(b).container.addEventListener("input", v, !0), s(b).container.addEventListener("change", v, !0), o(b).addEventListener("input", p), o(b).addEventListener("change", p), t.attachEvent("onResourcesSelectActivated", t.bind(p, o(b))), e.eventsInitialized[b.id] = !0;
      var w, $, x;
    }(l, u, c, this), _t(c.options, function(k, m) {
      c.unassigned_value != k.key && (h = function(b, g, v) {
        var p = {};
        if (g) {
          var y;
          Lt(g) ? y = Ee(g, function(w) {
            return w.resource_id == v.key;
          }) : g.resource_id == v.key && (y = g), y && (p.value = y.value, p.id = y.id);
        }
        return e.resourcesValues[b.id] && e.resourcesValues[b.id][v.key] && (p.value = e.resourcesValues[b.id][v.key].value, p.id = e.resourcesValues[b.id][v.key].id), p;
    ...
    for (var c = o(u), h = [], _ = r(!0), f = r(!1), k = s(u), m = t.copy(e.resourcesValues[u.id]) || {}, b = c.querySelectorAll(_), g = c.querySelectorAll(f), v = 0; v < g.length; v++) delete m[g[v].getAttribute("data-item-id")];
    for (v = 0; v < b.length; v++) {
      var p = b[v].getAttribute("data-assignment-id"), y = b[v].getAttribute("data-item-id"), w = b[v].value.trim();
      w !== "" && w !== "0" && (delete m[y], h[h.length] = { resource_id: y, value: w }, p && (h[h.length - 1] = { ...h[h.length - 1], id: p }));
    }
    if (k.filterApplied) for (var $ in m) h[h.length] = { resource_id: $, value: m[$].value, id: m[$].id };
    return h;
...
function Fr(t) {
  var n = function() {
    ...
    return W(p, v), p.prototype.render = function(y) {
      const w = (y.height || "130") + "px", $ = y.placeholder ? `placeholder='${y.placeholder}'` : "";
      return `<div class='gantt_cal_ltext gantt_section_${y.name}' style='height:${w};' ${$}><textarea></textarea></div>`;
    ...
    return W(p, v), p.prototype.render = function(y) {
      ...
      y.name !== "time" && ($ += " gantt_section_" + y.name);
      var x = "<div style='padding-top:0px;font-size:inherit;text-align:center;' class='" + $ + "'>";
      return x += w, y.single_date ? (w = g.form_blocks.getTimePicker.call(this, y, !0), x += "<span></span>") : x += "<span class='gantt_section_time_spacer'> &nbsp;&ndash;&nbsp; </span>", (x += w) + "</div>";
    }, p.prototype.set_value = function(y, w, $, x) {
      var S = x, T = y.getElementsByTagName("select"), E = x._time_format_order;
      if (S.auto_end_date) for (var C = function() {
        A = new Date(T[E[2]].value, T[E[1]].value, T[E[0]].value, 0, 0), I = g.calculateEndDate({ start_date: A, duration: 1, task: $ }), g.form_blocks._fill_lightbox_select(T, E.size, I, E, S);
      }, D = 0; D < 4; D++) T[D].onchange = C;
      var M = g._resolve_default_mapping(x);
      typeof M == "string" && (M = { start_date: M });
      var A = $[M.start_date] || /* @__PURE__ */ new Date(), I = $[M.end_date] || g.calculateEndDate({ start_date: A, duration: 1, task: $ });
      g.form_blocks._fill_lightbox_select(T, 0, A, E, S), g.form_blocks._fill_lightbox_select(T, E.size, I, E, S);
    }, p.prototype.get_value = function(y, w, $) {
      var x, S = y.getElementsByTagName("select"), T = $._time_format_order;
      return x = g.form_blocks.getTimePickerValue(S, $), typeof g._resolve_default_mapping($) == "string" ? x : { start_date: x, end_date: function(E, C, D) {
    ...
    return W(p, v), p.prototype.render = function(y) {
      ...
      let $ = `<div class='gantt_cal_ltext gantt_cal_lcheckbox gantt_section_${y.name}' ${w ? `style='${w}'` : ""}>`;
      if (y.options && y.options.length) for (var x = 0; x < y.options.length; x++) $ += "<label><input type='checkbox' value='" + y.options[x].key + "' name='" + y.name + "'>" + y.options[x].label + "</label>";
      else y.single_value = !0, $ += "<label><input type='checkbox' name='" + y.name + "'></label>";
      return $ += "</div>", $;
    }, p.prototype.set_value = function(y, w, $, x) {
      var S = Array.prototype.slice.call(y.querySelectorAll("input[type=checkbox]"));
      !y._dhx_onchange && x.onchange && (y.onchange = x.onchange, y._dhx_onchange = !0), x.single_value ? S[0].checked = !!w : _t(S, function(T) {
        T.checked = !!w && w.indexOf(T.value) >= 0;
      });
    }, p.prototype.get_value = function(y, w, $) {
      return $.single_value ? y.querySelector("input[type=checkbox]").checked : function(x, S) {
        if (x.map) return x.map(S);
        for (var T = x.slice(), E = [], C = 0; C < T.length; C++) E.push(S(T[C], C));
        return E;
      }(Array.prototype.slice.call(y.querySelectorAll("input[type=checkbox]:checked")), function(x) {
        return x.value;
      });
    }, p.prototype.focus = function(y) {
    ...
    return W(p, v), p.prototype.render = function(y) {
      ...
      let $ = `<div class='gantt_cal_ltext gantt_cal_lradio gantt_section_${y.name}' ${w ? `style='height:${w};'` : ""}>`;
      if (y.options && y.options.length) for (var x = 0; x < y.options.length; x++) $ += "<label><input type='radio' value='" + y.options[x].key + "' name='" + y.name + "'>" + y.options[x].label + "</label>";
      return $ += "</div>", $;
    }, p.prototype.set_value = function(y, w, $, x) {
      var S;
      x.options && x.options.length && (S = y.querySelector("input[type=radio][value='" + w + "']") || y.querySelector("input[type=radio][value='" + x.default_value + "']")) && (!y._dhx_onchange && x.onchange && (y.onchange = x.onchange, y._dhx_onchange = !0), S.checked = !0);
    }, p.prototype.get_value = function(y, w) {
      var $ = y.querySelector("input[type=radio]:checked");
      return $ ? $.value : "";
    }, p.prototype.focus = function(y) {
    ...
    return W(p, v), p.prototype.render = function(x) {
      var S = "<div class='gantt_time_selects'>" + g.form_blocks.getTimePicker.call(this, x) + "</div>", T = " " + g.locale.labels[g.config.duration_unit + "s"] + " ", E = x.single_date ? " style='display:none'" : "", C = x.readonly ? " disabled='disabled'" : "", D = g._waiAria.lightboxDurationInputAttrString(x), M = "gantt_duration_value";
      x.formatter && (T = "", M += " gantt_duration_value_formatted");
      ...
      return x.name !== "time" && (I += " gantt_section_" + x.name), "<div style='padding-top:0px;font-size:inherit;' class='" + I + "'>" + S + " " + A + "</div>";
    }, p.prototype.set_value = function(x, S, T, E) {
      var C, D, M, A, I = x.getElementsByTagName("select"), L = x.getElementsByTagName("input"), P = L[1], N = [L[0], L[2]], O = x.getElementsByTagName("span")[0], B = E._time_format_order;
      function H() {
        var V = w.call(g, x, E), j = $.call(g, x, E), rt = g.calculateEndDate({ start_date: V, duration: j, task: T }), vt = g.templates.task_end_date || g.templates.task_date;
        O.innerHTML = vt(rt);
      }
      function F(V) {
        var j = P.value;
      ...
      }, this), P.onchange = g.bind(H, this), typeof (C = g._resolve_default_mapping(E)) == "string" && (C = { start_date: C }), D = T[C.start_date] || /* @__PURE__ */ new Date(), M = T[C.end_date] || g.calculateEndDate({ start_date: D, duration: 1, task: T }), A = Math.round(T[C.duration]) || g.calculateDuration({ start_date: D, end_date: M, task: T }), A = y(E).format(A), g.form_blocks._fill_lightbox_select(I, 0, D, B, E), P.value = A, H();
    }, p.prototype.get_value = function(x, S, T) {
      var E = w(x, T), C = $(x, T), D = g.calculateEndDate({ start_date: E, duration: C, task: S });
    ...
    function w($, x) {
      for (var S = y(x), T = 0; T < $.length; T++) $[T].disabled = S;
    }
    return W(p, v), p.prototype.render = function($) {
      const x = $.height ? `height:${$.height}px;` : "";
      let S = `<div class='gantt_cal_ltext gantt_section_${$.name}' ${x ? `style='${x}'` : ""}>`;
      var T = [];
      ...
      return $.options = $.options || T, S += "<span data-constraint-type-select>" + ae.getHtmlSelect($.options, [{ key: "data-type", value: "constraint-type" }]) + "</span>", S += "<label data-constraint-time-select>" + (g.locale.labels.constraint_date || "Constraint date") + ": " + g.form_blocks.getTimePicker.call(this, $) + "</label>", S += "</div>", S;
      ...
      g.form_blocks._fill_lightbox_select(C, 0, A, D, T);
      var I = S[M.constraint_type] || g.getConstraintType(S);
      E.value = I, w(C, I);
    ...
    return W(p, v), p.prototype.render = function(y) {
      var w = g.config.types, $ = g.locale.labels, x = [], S = y.filter || function(C, D) {
        return !w.placeholder || D !== w.placeholder;
      };
      for (var T in w) !S(T, w[T]) == 0 && x.push({ key: w[T], label: $["type_" + T] });
      y.options = x;
      var E = y.onchange;
    ...
    function w(S, T, E, C) {
      ...
      const A = C.single_date ? " style='display:none'" : "", I = C.readonly ? " disabled='disabled'" : "", L = g._waiAria.lightboxDurationInputAttrString(C), P = g.locale.labels.baselines_remove_button;
      let N = "gantt_duration_value";
      C.formatter && (M = "", N += " gantt_duration_value_formatted");
      const O = "<div class='gantt_duration' " + A + "><div class='gantt_duration_inputs'><input type='button' class='gantt_duration_dec' value='−'" + I + "><input type='text' value='5days' class='" + N + "'" + I + " " + L + "><input type='button' class='gantt_duration_inc' value='+'" + I + "></div><div class='gantt_duration_end_date'>" + M + "<span></span></div></div></div>", B = `<div><div class='baseline_delete_button gantt_custom_button'>${P}</div></div>`, H = document.createElement("div");
      H.className = "gantt_section_time gantt_section_duration", H.setAttribute("data-baseline-id", T.id), H.innerHTML = D + O + B + "<br>", S.appendChild(H);
      var F, V, j, rt = H.getElementsByTagName("select"), vt = H.getElementsByTagName("input"), q = vt[1], St = [vt[0], vt[2]], Ht = H.getElementsByTagName("span")[0], Tt = C._time_format_order;
      function $t() {
        var kt = $.call(g, H, C), X = x.call(g, H, C), ei = g.calculateEndDate({ start_date: kt, duration: X, task: E }), ni = g.templates.task_end_date || g.templates.task_date;
        Ht.innerHTML = ni(ei);
      }
      function qt(kt) {
        var X = q.value;
        X = y(C).parse(X), window.isNaN(X) && (X = 0), (X += kt) < 1 && (X = 1), q.value = y(C).format(X), $t();
      }
      H.querySelector(".baseline_delete_button").onclick = function(kt) {
        ...
        qt(1 * g.config.duration_step);
      }, g), rt[0].onchange = $t, rt[1].onchange = $t, rt[2].onchange = $t, rt[3] && (rt[3].onchange = $t), q.onkeydown = g.bind(function(kt) {
        var X;
        return (X = (kt = kt || window.event).charCode || kt.keyCode || kt.which) == g.constants.KEY_CODES.DOWN ? (qt(-1 * g.config.duration_step), !1) : X == g.constants.KEY_CODES.UP ? (qt(1 * g.config.duration_step), !1) : void window.setTimeout($t, 1);
      }, g), q.onchange = g.bind($t, g), g._resolve_default_mapping(C), F = T.start_date || /* @__PURE__ */ new Date(), V = T.end_date || g.calculateEndDate({ start_date: F, duration: 1, task: E }), j = g.calculateDuration({ start_date: F, end_date: V, task: E }), j = y(C).format(j), g.form_blocks._fill_lightbox_select(rt, 0, F, Tt, C), q.value = j, $t();
    }
    function $(S, T) {
    ...
    return W(p, v), p.prototype.render = function(S) {
      ...
      return S.querySelectorAll("[data-baseline-id]").forEach((D) => {
        ...
        let A, I = g.getDatastore("baselines").getItem(M);
        A = I ? g.copy(I) : { id: g.uid(), task_id: T.id, text: "Baseline 1" }, A.start_date = $(D, E), A.duration = x(D, E), A.end_date = g.calculateEndDate({ start_date: A.start_date, duration: A.duration, task: T }), C.push(A);
      }), C;
...
<div class="gantt_cal_ltitle_controls">
    ...
    var v = this.getTask(g);
    if (this.callEvent("onBeforeLightbox", [g])) {
      var p = this.getLightbox(this.getTaskType(v.type));
    ...
    if (this.config.round_dnd_dates) {
      ...
      if (function(p) {
        ...
        var v = t.getScale();
        g = Zt(v.unit) * v.step / 60;
      }
      return (!g || g >= 1440) && (g = this.config.time_step), g;
    }
    return this.config.time_step;
  }, t.getLabel = function(g, v) {
    for (var p = this._get_typed_lightbox_config(), y = 0; y < p.length; y++) if (p[y].map_to == g) {
      for (var w = p[y].options, $ = 0; $ < w.length; $++) if (w[$].key == v) return w[$].label;
    }
    return "";
  }, t.updateCollection = function(g, v) {
    v = v.slice(0);
    var p = t.serverList(g);
    if (!p) return !1;
    p.splice(0, p.length), p.push.apply(p, v || []), t.resetLightbox();
    ...
    if (function() {
      ...
      this._lightbox_type = this.getTaskType(g), v = document.createElement("div"), x = "gantt_cal_light", p = this._is_lightbox_timepicker(), t.config.wide_form && (x += " gantt_cal_light_wide"), p && (x += " gantt_cal_light_full"), v.className = x, v.style.visibility = "hidden", y = this._lightbox_template, y += "<div class='gantt_cal_lcontrols'>", y += k(this.config.buttons_left), y += "<div class='gantt_cal_lcontrols_push_right'></div>", y += k(this.config.buttons_right), y += "</div>", v.innerHTML = y, t._waiAria.lightboxAttr(v), t.config.drag_lightbox && (v.firstChild.onmousedown = t._ready_to_dnd, v.firstChild.ontouchstart = function(T) {
        t._ready_to_dnd(T.touches[0]);
      }, v.firstChild.onselectstart = function() {
        return !1;
      }, v.firstChild.style.cursor = "pointer", t._init_dnd_events()), this._lightbox && this.resetLightbox(), _(), this._cover.insertBefore(v, this._cover.firstChild), this._lightbox = v, w = this._get_typed_lightbox_config(g), y = this._render_sections(w);
      var S = ($ = v.querySelector("div.gantt_cal_larea")).style.overflow;
      $.style.overflow = "hidden", $.innerHTML = y, function(T) {
        var E, C, D, M, A, I;
      ...
      }(w), $.style.overflow = S, this._init_lightbox_events(this), v.style.display = "none", v.style.visibility = "visible";
    }
    ...
    for (var v = "", p = 0; p < g.length; p++) {
      ...
      if (y) {
        ...
        g[p].button && ($ = "<div class='gantt_custom_button' data-index='" + p + "'><div class='gantt_custom_button_" + g[p].button + "'></div><div class='gantt_custom_button_label'>" + this.locale.labels["button_" + g[p].button] + "</div></div>"), g[p].type == "baselines" && ($ = "<div class='gantt_custom_button gantt_remove_baselines' data-index='" + p + "'><div class='gantt_custom_button_delete_baselines'></div><div class='gantt_custom_button_label'>" + this.locale.labels.baselines_remove_all_button + "</div></div><div class='gantt_custom_button gantt_add_baselines' data-index='" + p + "'><div class='gantt_custom_button_add_baseline'></div><div class='gantt_custom_button_label'>" + this.locale.labels.baselines_add_button + "</div></div>"), this.config.wide_form && (v += "<div class='gantt_wrap_section' " + w + ">"), v += "<div id='" + g[p].id + "' class='gantt_cal_lsection'><label>" + $ + this.locale.labels["section_" + g[p].name] + "</label></div>" + y.render.call(this, g[p]), v += "</div>";
      }
    }
    return v;
    ...
    if (!g) return;
    const v = t._lightbox_root || t.$root;
    g.style.top = Math.max(v.offsetHeight / 2 - g.offsetHeight / 2, 0) + "px", g.style.left = Math.max(v.offsetWidth / 2 - g.offsetWidth / 2, 0) + "px";
  ...
  function f(g) {
    for (var v in this.config.types) if (this.config.types[v] == g) return v;
    return "task";
  }
  function k(g, v) {
    var p, y, w = "";
  ...
  function m(g) {
    var v, p;
    return g.time_format ? g.time_format : (p = ["%d", "%m", "%Y"], Zt((v = t.getScale()) ? v.unit : t.config.duration_unit) < Zt("day") && p.push("%H:%i"), p);
  }
  function b(g, v, p) {
    var y, w, $, x, S, T, E = "";
    switch (p.timeFormat[v]) {
      case "%Y":
        for (g._time_format_order[2] = v, g._time_format_order.size++, g.year_range && (isNaN(g.year_range) ? g.year_range.push && ($ = g.year_range[0], x = g.year_range[1]) : y = g.year_range), y = y || 10, w = w || Math.floor(y / 2), $ = $ || p.date.getFullYear() - w, x = x || t.getState().max_date.getFullYear() + w, S = $; S <= x; S++) E += "<option value='" + S + "'>" + S + "</option>";
        break;
      case "%m":
        for (g._time_format_order[1] = v, g._time_format_order.size++, S = 0; S < 12; S++) E += "<option value='" + S + "'>" + t.locale.date.month_full[S] + "</option>";
        break;
      case "%d":
        for (g._time_format_order[0] = v, g._time_format_order.size++, S = 1; S < 32; S++) E += "<option value='" + S + "'>" + S + "</option>";
        break;
      case "%H:%i":
        for (g._time_format_order[3] = v, g._time_format_order.size++, S = p.first, T = p.date.getDate(), g._time_values = []; S < p.last; ) E += "<option value='" + S + "'>" + t.templates.time_picker(p.date) + "</option>", g._time_values.push(S), p.date.setTime(p.date.valueOf() + 60 * t._get_timepicker_step() * 1e3), S = 24 * (p.date.getDate() != T ? 1 : 0) * 60 + 60 * p.date.getHours() + p.date.getMinutes();
    }
    return E;
  }
  t._init_lightbox_events = function() {
    t.lightbox_events = {}, t.lightbox_events.gantt_save_btn = function() {
      ...
      t._cancel_lightbox();
    }, t.lightbox_events.default = function(g, v) {
      if (v.getAttribute("data-dhx-button")) t.callEvent("onLightboxButton", [v.className, v, g]);
      else {
        var p, y, w = nt(v);
        if (w.indexOf("gantt_custom_button") != -1) if (w.indexOf("gantt_custom_button_") != -1) for (p = v.parentNode.getAttribute("data-index"), y = v; y && nt(y).indexOf("gantt_cal_lsection") == -1; ) y = y.parentNode;
        else p = v.getAttribute("data-index"), y = v.closest(".gantt_cal_lsection"), v = v.firstChild;
        var $ = t._get_typed_lightbox_config();
        p && (p *= 1, t.form_blocks[$[1 * p].type].button_click(p, v, y, y.nextSibling));
      }
    }, this.event(t.getLightbox(), "click", function(g) {
      g.target.closest(".gantt_cal_ltitle_close_btn") && t._cancel_lightbox();
      var v = At(g), p = nt(v);
      return p || (p = nt(v = v.previousSibling)), v && p && p.indexOf("gantt_btn_set") === 0 && (p = nt(v = v.firstChild)), !(!v || !p) && (t.defined(t.lightbox_events[v.className]) ? t.lightbox_events[v.className] : t.lightbox_events.default)(g, v);
    }), t.getLightbox().onkeydown = function(g) {
      var v = g || window.event, p = g.target || g.srcElement, y = nt(p).indexOf("gantt_btn_set") > -1;
      switch ((g || v).keyCode) {
        case t.constants.KEY_CODES.SPACE:
          if ((g || v).shiftKey) return;
          y && p.click && p.click();
          break;
    ...
    var v = g.map_to;
    return { time: !0, time_optional: !0, duration: !0, duration_optional: !0 }[g.type] ? g.map_to == "auto" ? v = { start_date: "start_date", end_date: "end_date", duration: "duration" } : typeof g.map_to == "string" && (v = { start_date: g.map_to }) : g.type === "constraint" && (g.map_to && typeof g.map_to != "string" || (v = { constraint_type: "constraint_type", constraint_date: "constraint_date" })), v;
  }, t.getLightboxValues = function() {
    var g = {};
    t.isTaskExists(this._lightbox_id) && (g = this.mixin({}, this.getTask(this._lightbox_id)));
    for (var v = this._get_typed_lightbox_config(), p = 0; p < v.length; p++) {
      var y = t._lightbox_root.querySelector("#" + v[p].id);
      y = y && y.nextSibling;
      var w = this.form_blocks[v[p].type];
      if (w) {
        var $ = w.get_value.call(this, y, g, v[p]), x = t._resolve_default_mapping(v[p]);
        if (typeof x == "string" && x != "auto") g[x] = $;
        else if (typeof x == "object") for (var S in x) x[S] && (g[x[S]] = $[S]);
      }
    }
  ...
  }, t._fill_lightbox = function(g, v) {
    var p = this.getTask(g);
    this._set_lightbox_values(p, v);
  }, t.getLightboxSection = function(g) {
    for (var v = this._get_typed_lightbox_config(), p = 0; p < v.length && v[p].name != g; p++) ;
    var y = v[p];
    if (!y) return null;
    this._lightbox || this.getLightbox();
    var w = t._lightbox_root.querySelector("#" + y.id), $ = w.nextSibling, x = { section: y, header: w, node: $, getValue: function(T) {
      return t.form_blocks[y.type].get_value.call(t, $, T || {}, y);
    }, setValue: function(T, E) {
      return t.form_blocks[y.type].set_value.call(t, $, T, E || {}, y);
    ...
    this.event(g, "mousemove", t._move_while_dnd), this.event(g, "mouseup", t._finish_dnd), this.event(g, "touchmove", function(v) {
      t._move_while_dnd(v.touches[0]);
    }), this.event(g, "touchend", function(v) {
      t._finish_dnd(v.touches[0]);
    });
    ...
    if (t._dnd_start_lb) {
      ...
      var v = t.getLightbox(), p = [g.pageX, g.pageY];
      v.style.top = t._lb_start[1] + p[1] - t._dnd_start_lb[1] + "px", v.style.left = t._lb_start[0] + p[0] - t._dnd_start_lb[0] + "px";
    }
  }, t._ready_to_dnd = function(g) {
    var v = t.getLightbox();
    t._lb_start = [v.offsetLeft, v.offsetTop], t._dnd_start_lb = [g.pageX, g.pageY];
  ...
  }, t._focus = function(g, v) {
    if (g && g.focus && !t.config.touch) try {
      v && g.select && g.select(), g.focus();
    } catch {
    }
  }, t.form_blocks = { getTimePicker: function(g, v) {
    var p, y, w, $ = "", x = this.config, S = { first: 0, last: 1440, date: this.date.date_part(new Date(t._min_date.valueOf())), timeFormat: m(g) };
    for (g._time_format_order = { size: 0 }, t.config.limit_time_select && (S.first = 60 * x.first_hour, S.last = 60 * x.last_hour + 1, S.date.setHours(x.first_hour)), p = 0; p < S.timeFormat.length; p++) p > 0 && ($ += " "), (y = b(g, p, S)) && (w = t._waiAria.lightboxSelectAttrString(S.timeFormat[p]), $ += "<select " + (g.readonly ? "disabled='disabled'" : "") + (v ? " style='display:none' " : "") + w + ">" + y + "</select>");
    return $;
  }, getTimePickerValue: function(g, v, p) {
    var y, w = v._time_format_order, $ = 0, x = 0, S = p || 0;
    return t.defined(w[3]) && (y = parseInt(g[w[3] + S].value, 10), $ = Math.floor(y / 60), x = y % 60), new Date(g[w[2] + S].value, g[w[1] + S].value, g[w[0] + S].value, $, x);
  }, _fill_lightbox_select: function(g, v, p, y) {
    if (g[v + y[0]].value = p.getDate(), g[v + y[1]].value = p.getMonth(), g[v + y[2]].value = p.getFullYear(), t.defined(y[3])) {
    ...
    for (var g = this._get_typed_lightbox_config(), v = 0; v < g.length; v++) if (g[v].name == "time" && g[v].type == "time") return !0;
    return !1;
  }, t._simple_confirm = function(g, v, p, y) {
    if (!g) return p();
    var w = { text: g };
    v && (w.title = v), y && (w.ok = y), p && (w.callback = function($) {
      $ && p();
    }), t.confirm(w);
  }, t._get_typed_lightbox_config = function(g) {
    g === void 0 && (g = this.getLightboxType());
    var v = f.call(this, g);
    return t.config.lightbox[v + "_sections"] ? t.config.lightbox[v + "_sections"] : t.config.lightbox.sections;
  }, t._silent_redraw_lightbox = function(g) {
    var v = this.getLightboxType();
    if (this.getState().lightbox) {
      var p = this.getState().lightbox, y = this.getLightboxValues(), w = this.copy(this.getTask(p));
      this.resetLightbox();
      var $ = this.mixin(w, y, !0), x = this.getLightbox(g || void 0);
      this._set_lightbox_values($, x), this.showCover(x);
    } else this.resetLightbox(), this.getLightbox(g || void 0);
    this.callEvent("onLightboxChange", [v, this.getLightboxType()]);
  };
}
function Wr(t) {
  if (!bt.isNode) {
    ...
    var n = We();
    t.event = n.attach, t.eventRemove = n.detach, t._eventRemoveAll = n.detachAll, t._createDomEventScope = n.extend, R(t, Ba(t));
    var e = Mr.init(t);
    t.$ui = e.factory, t.$ui.layers = e.render, t.$mouseEvents = e.mouseEvents, t.$services.setService("mouseEvents", function() {
      ...
      return { getColumnIndex: function(c) {
        ...
        var _ = o(this), f = s(this), k = { position: 0 }, m = { position: 0 };
        _ && (m = _.getScrollState()), f && (k = f.getScrollState());
        var b = f && 1 * c == c, g = _ && 1 * h == h;
        if (b && g) for (var v = _._getLinkedViews(), p = f._getLinkedViews(), y = [], w = 0; w < v.length; w++) for (var $ = 0; $ < p.length; $++) v[w].$config.id && p[$].$config.id && v[w].$config.id === p[$].$config.id && y.push(v[w].$config.id);
        ...
        _ && (x = _.getScrollState()), f && (S = f.getScrollState()), this.callEvent("onGanttScroll", [k.position, m.position, S.position, x.position]);
      }, showDate: function(c) {
        ...
        var f, k = Math.max(_ - this.config.task_scroll_offset, 0), m = this._scroll_state().y;
        f = m ? h.top - (m - this.getTaskBarHeight(c)) / 2 : h.top, this.scrollTo(k, f);
      ...
      i.resetSkin || (i.resetSkin = function() {
        ...
        this.skin = o, i._addThemeClass(), r(), i.$root && (we(!0, i), this.render());
      };
      let a = null;
      function r() {
        const o = i.$root;
      ...
      i.ext || (i.ext = {});
      for (var a = [Lr, Nr, Pr], r = 0; r < a.length; r++) a[r] && a[r](i);
      i.ext.zoom = new Or(i);
      ...
      var r = arguments[1] || this.config.task_attribute, o = Q(a, r);
  ...
  t.attachEvent("onParse", function() {
    ...
    this.config.scroll_size || (this.config.scroll_size = En() || 15), Y(t) || (this._eventRemoveAll(), this.$mouseEvents.reset(), this.resetLightbox());
  }), t.attachEvent("onGanttReady", function() {
    !Y(t) && t.config.rtl && t.$layout.getCellsByType("viewCell").forEach(function(i) {
